...
class c_admin_x11 extends c_base {
	function index($where = NULL, $interface=false) {
		...
		if($delete) {
			...
			$this->display("x11session_list.tpl");
		}
	}
	...
	function inputview(){
		...
		if(empty($logfile)){
			...
			if($start_page>500)
			{
				print "<a href=\"admin.php?controller=admin_x11&action=inputview&sid=$sid&start_page=$pre_page\">pre page<br></a>";
			}

			if(count($buffer_array)==500)
			{
				print "&nbsp &nbsp &nbsp <a href=\"admin.php?controller=admin_x11&action=inputview&sid=$sid&start_page=$next_page\">next page</a><br>";
			}
			echo language('键盘记录如下').':<br />';
