...
class c_admin_apppubrun extends c_base {
	function index($where = NULL, $r=0, $pre='') {
		...
		$sid = get_request('sid');
		$where = "LOGIN_TEMPLATE=26 and rdp_runnig=1";
		...
		if($delete) {
			...
			$this->display("apppubsessionrun_list.tpl");
		}
	}
	...
	function get_eth0_ip() {
		...
		$return['eth0'] = trim($_SERVER["HTTP_HOST"]);
		unset($lines);
	...
	function mstsc($sid){
		...
		$s['proxy_addr'] = $eth0;
		if($s['device_ip']!='127.0.0.1'){
			$s['device_ip']=$s['proxy_addr'];
		}
	...
	function inputview(){
		...
		if(empty($logfile)){
			...
			$value   =   file($logfile);
			$buffer_array=array();
	        $tmp1 = exec("sed  p $logfile",$buffer_array,$return);
