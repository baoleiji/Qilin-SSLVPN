...
class c_admin_sqlserver extends c_base {
	function index($where = NULL) {
		...
		if($_SESSION['ADMIN_LEVEL'] == 0 ) {
			$where .= " AND d_addr IN (SELECT device_ip FROM ".$this->devpass_set->get_table_name()." WHERE id IN (".implode(",",$alldevsid).")) ";
		}
		elseif($_SESSION['ADMIN_LEVEL'] == 3 || $_SESSION['ADMIN_LEVEL'] == 21 || $_SESSION['ADMIN_LEVEL'] == 101) {
			...
			$where .= " AND d_addr IN (SELECT device_ip FROM ".$this->server_set->get_table_name()." WHERE id IN ($_SESSION[DEVS]))";
		}
		...
		if($command) {
			$where .= " AND (SELECT COUNT(*) FROM `sqlserver_commands` WHERE cmd LIKE '%$command%' AND sqlserver_commands.sid = sqlserver_sessions.sid) > 0";
		}
		...
		if($delete) {
			...
			$row_num = $this->sqlserver_set->select_count($where);
			...
			$this->assign('allsession', $this->sqlserver_set->select_limit($newpager->intStartPosition, $newpager->intItemsPerPage, $where, $orderby1, $orderby2));

			//$this->assign('now_table_name', $table_name);
			//$this->assign('table_list', $table_list);
			$this->display("sqlserversession_list.tpl");
		}
	}
	...
	function search() {
		...
		$this->display("sqlserver_search.tpl");
	}
	...
	function view() {
		...
		$row_num = $this->sqlservercommands_set->select_count($where);
		$newpager = new my_pager($row_num, $page_num, $this->config['site']['items_per_page'], 'page');
		$this->assign('page_list', $newpager->showSerialList());
		...
		$allcommand =  $this->sqlservercommands_set->select_limit($newpager->intStartPosition, $newpager->intItemsPerPage, $where, '', 'asc');
	...
	function del_command() {
		$cid = get_request('cid');
		$this->sqlservercommands_set->delete($cid);
		alert_and_back('删除成功');
	}
	...
	function del_session() {
		global $_CONFIG;
		$sid = get_request('sid');
		$session = $this->sqlserver_set->select_by_id($sid);
		if ($this->sqlserver_set->select_count("logfile = '".str_replace('\'','\\\'',$session['logfile'])."'") == 1 ) {
			if(file_exists($_CONFIG['ORACLE_LOG_PATH_PREFIX'].$session['logfile'])) {
				unlink($_CONFIG['ORACLE_LOG_PATH_PREFIX'].$session['logfile']);
			}
		}
		$this->sqlserver_set->delete($sid);
		$this->sqlservercommands_set->query("DELETE FROM `" . $this->sqlservercommands_set->get_table_name() . "` WHERE sid = '$sid'");
		alert_and_back('删除成功!');
	}
	...
	function download() {
		...
		$sessions = $this->sqlserver_set->select_by_id($sid);
	...
	function replay() {
		...
		$session = $this->sqlserver_set->select_by_id($sid);
		$cmd = "/opt/freesvr/audit//gateway//log/bin/freesvr_db_replay.pl sqlserver ".$sid;
		exec($cmd);
