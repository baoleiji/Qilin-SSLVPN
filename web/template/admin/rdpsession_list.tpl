...
function searchit(){
	var url = "admin.php?controller=admin_rdp&backupdb_id={{$backupdb_id}}";
...
<li class="me_b"><img src="{{$template_root}}/images/an11.jpg" align="absmiddle"/><a href="admin.php?controller=admin_workflow&action=workflow_approve">流程审批</a><img src="{{$template_root}}/images/an33.jpg" align="absmiddle"/></li> 
  ...
  <tr>
         <td >
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="main_content"><form action="admin.php?controller=admin_rdp&backupdb_id={{$backupdb_id}}" method="post" name="search" >
  <tr>
    <td></td>
    <td>
...
cal.manageFields("f_rangeEnd_trigger", "f_rangeEnd", "%Y-%m-%d %H:%M:%S");
  ...
  <tr><td><table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
					<tr>
						<th class="list_bg"   width="9%"><a href="admin.php?controller=admin_rdp&orderby1=cli_addr&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">来源地址</a></th>
						<th class="list_bg"   width="9%"><a href="admin.php?controller=admin_rdp&orderby1=addr&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">设备地址</a></th>
						
						<th class="list_bg"   width="6%"><a href="admin.php?controller=admin_rdp&orderby1=luser&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">运维</a></th>
						<th class="list_bg"   width="6%"><a href="admin.php?controller=admin_rdp&orderby1=realname&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">真实姓名</a></th>
						<th class="list_bg"   width="7%"><a href="admin.php?controller=admin_rdp&orderby1=user&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">本地</a></th>
						<th class="list_bg"   width="11%"><a href="admin.php?controller=admin_rdp&orderby1=start&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">开始时间</a></th>
						<th class="list_bg"   width="11%"><a href="admin.php?controller=admin_rdp&orderby1=end&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">结束时间</a></th>
						<th class="list_bg"   width="5%"><a href="admin.php?controller=admin_rdp&orderby1=filesize&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">文件(K)</a></th>
					...
					<tr {{if $allsession[t].dangerous > 1}}bgcolor="red"{{elseif $allsession[t].dangerous > 0}}bgcolor="yellow" {{elseif $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}} onmouseover="changeStyle(this,'o');" onmouseout="changeStyle(this,'{{if $allsession[t].dangerous > 1}}red{{elseif $allsession[t].dangerous > 0}}yellow{{elseif $smarty.section.t.index % 2 == 0}}f7f7f7{{/if}}');">
										...
										<td><a href="admin.php?controller=admin_rdp&srcaddr={{$allsession[t].cli_addr}}&backupdb_id={{$backupdb_id}}">{{$allsession[t].cli_addr}}</a></td>
					
					<td><a href="admin.php?controller=admin_rdp&addr={{$allsession[t].addr}}&backupdb_id={{$backupdb_id}}">{{$allsession[t].addr}}</a></td>
					
					<td><a href="admin.php?controller=admin_rdp&luser={{$allsession[t].luser}}&backupdb_id={{$backupdb_id}}">{{$allsession[t].luser}}</a></td>
					<td><a href="admin.php?controller=admin_rdp&realname={{$allsession[t].realname|urlencode}}&backupdb_id={{$backupdb_id}}">{{$allsession[t].realname}}</a></td>
					<td><a href="admin.php?controller=admin_rdp&user={{$allsession[t].user}}&backupdb_id={{$backupdb_id}}">{{$allsession[t].user}}</a></td>
					...
					<a  id="p_{{$allsession[t]['sid']}}" onClick="return go('{{$curr_url}}&mstsc=1&sid={{$allsession[t]['sid']}}','p_{{$allsession[t]['sid']}}')" href="#" target="hide">回放</a>
						...
						&nbsp;| <img src="{{$template_root}}/images/input.gif" width="16" height="16" align="absmiddle">{{if $rdpinput}}<a href="{{$curr_url}}&activex=1&sid={{$allsession[t]['sid']}}&action=inputview" target="_blank">录入</a>{{else}}录入{{/if}}
						&nbsp;| <img src="{{$template_root}}/images/mouse.png" width="16" height="16" align="absmiddle"><a href="{{$curr_url}}&activex=1&sid={{$allsession[t]['sid']}}&action=rdpmouse" >鼠标</a>
						&nbsp;| <img src="{{$template_root}}/images/i_4.gif" width="16" height="16" align="absmiddle"><a href="{{$curr_url}}&activex=1&sid={{$allsession[t]['sid']}}&action=clipboard" target="_blank">剪贴板</a>
						{{else}}<img src="{{$template_root}}/images/file.gif" width="16" height="16" align="absmiddle"> <a href="#" onclick='window.open("admin.php?controller=admin_rdp&action=download&sid={{$allsession[t]['sid']}}&start_page=1&command={{$command}}&backupdb_id={{$backupdb_id}}");return false;'>{{$language.File}}</a>&nbsp;&nbsp;{{/if}} 
						&nbsp;&nbsp;<img src="{{$template_root}}/images/1-1.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_session&desc={{$allsession[t].desc}}&action=logindesc&type=rdp&sessionid={{$allsession[t]['sid']}}" target="hide" ><font style="color:{{if $allsession[t].desc}}red{{else}}black{{/if}}">备注</font></a>
						...
						{{if 0}} | <img src="{{$template_root}}/images/scico.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_rdp&action=del_session&sid={{$allsession[t]['sid']}}">删除</a>{{/if}}</td>
					</tr>
					{{/section}}
					<tr>
