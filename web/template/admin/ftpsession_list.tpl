...
function searchit(){
	var url = "admin.php?controller=admin_ftp&backupdb_id={{$backupdb_id}}";
...
<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_ftp&backupdb_id={{$backupdb_id}}">FTP</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li> 
...
<li class="me_b"><img src="{{$template_root}}/images/an11.jpg" align="absmiddle"/><a href="admin.php?controller=admin_workflow&action=workflow_approve">流程审批</a><img src="{{$template_root}}/images/an33.jpg" align="absmiddle"/></li> 
   ...
   <tr>
       ...
       <td >
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="main_content"><form action="admin.php?controller=admin_ftp&backupdb_id={{$backupdb_id}}" method="post" name="search" >
  <tr>
    <td></td>
    <td>
...
<input  type="text" class="wbk" name="f_rangeEnd" size="13" id="f_rangeEnd" value="" />
 <input type="button" onClick="changetype('timetype3')" id="f_rangeEnd_trigger" name="f_rangeEnd_trigger" value="选择时间" class="wbk">
	  &nbsp;&nbsp;<input type="submit" height="35" align="middle" onClick="return searchit();" border="0" value=" 确定 " class="bnnew2"/>&nbsp;&nbsp;<input id="filtercheck" type="checkbox" onclick="filter1();" name="filter" value="1" {{if $filter}}checked{{/if}} >过滤
 </td>
  </tr></form>
</table>
...
cal.manageFields("f_rangeEnd_trigger", "f_rangeEnd", "%Y-%m-%d %H:%M:%S");
  ...
  <tr><td><table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
					<tr>
						...
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_ftp&orderby1=cliaddr&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">来源地址</a></th>
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_ftp&orderby1=svraddr&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">{{$language.DestinationAddress}}</a></th>
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_ftp&orderby1=radius_user&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">运维</a></th>	
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_ftp&orderby1=realname&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">真实姓名</a></th>
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_ftp&orderby1=ftp_user&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">{{$language.LocalUser}}</a></th>					
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_ftp&orderby1=start&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">{{$language.StartTime}}</a></th>
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_ftp&orderby1=end&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">{{$language.EndTime}}</a></th>
						<th class="list_bg"   width="18%"><a href="admin.php?controller=admin_ftp&orderby1=addr&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">{{$language.Detail}}</th>
					</tr>
					...
					<td><a href="admin.php?controller=admin_ftp&cliaddr={{$allsession[t].cliaddr}}&backupdb_id={{$backupdb_id}}">{{$allsession[t].cliaddr}}</a></td>
						
						<td><a href="admin.php?controller=admin_ftp&svraddr={{$allsession[t].svraddr}}&backupdb_id={{$backupdb_id}}">{{$allsession[t].svraddr}}</a></td>
						
						<td><a href="admin.php?controller=admin_ftp&radius_user={{$allsession[t].radius_user}}&backupdb_id={{$backupdb_id}}">{{$allsession[t].radius_user}}</a></td>
						<td><a href="admin.php?controller=admin_ftp&realname={{$allsession[t].realname|urlencode}}&backupdb_id={{$backupdb_id}}">{{$allsession[t].realname}}</a></td>
						<td><a href="admin.php?controller=admin_ftp&ftp_user={{$allsession[t].ftp_user}}&backupdb_id={{$backupdb_id}}">{{$allsession[t].ftp_user}}</a></td>
		
						<td>{{$allsession[t].start}}</ td>
						
						<td>{{$allsession[t].end}}</td>
						
						<td style="TEXT-ALIGN: left;"><img src="{{$template_root}}/images/ico2.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_ftp&action=view&sid={{$allsession[t]['sid']}}&command={{$command}}&backupdb_id={{$backupdb_id}}">{{$language.View}}</a>(命令条数:{{$allsession[t].total_cmd}})
						&nbsp;&nbsp;<img src="{{$template_root}}/images/1-1.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_session&desc={{$allsession[t].desc}}&action=logindesc&type=ftp&sessionid={{$allsession[t]['sid']}}" target="hide" ><font style="color:{{if $allsession[t].desc}}red{{else}}black{{/if}}">备注</font></a>
						{{if $allsession[t].logincommit}}&nbsp;&nbsp;<img src="{{$template_root}}/images/1-1.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_session&commit={{$allsession[t].logincommit}}&action=logincommit&type=ftp" target="hide" >说明</a>{{/if}}

						{{if 0}} {{if !$backupdb_id}}| <img src="{{$template_root}}/images/cross.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_ftp&action=del_session&sid={{$allsession[t]['sid']}}">{{$language.Delete}}</a>{{/if}}{{/if}}</td>
					</tr>
					{{/section}}
					<tr>
