	...
	<form name="member_list" action="admin.php?controller=admin_member&action=protect_lgroup_delete" method="post">
				<table bordercolor="white" cellspacing="1" cellpadding="5" border="0" width="100%"  class="BBtable">
					<tr>
						...
						<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_member&action=protect_group_devgrp&gid={{$gid}}&orderby1=groupname&orderby2={{$orderby2}}" >{{$language.DeviceGroup}}</a></th>
						<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_member&action=protect_group_devgrp&gid={{$gid}}&orderby1=weektime&orderby2={{$orderby2}}" >{{$language.WeekTimepolicy}}</a></th>
						<th class="list_bg"  width="15%"><a href="admin.php?controller=admin_member&action=protect_group_devgrp&gid={{$gid}}&orderby1=sourceip&orderby2={{$orderby2}}" >{{$language.SourceIP}}</a></th>
						<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_member&action=protect_group_devgrp&gid={{$gid}}&orderby1=forbidden_commands_groups&orderby2={{$orderby2}}" >{{$language.Black}}</a></th>
					...
					<tr {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						...
						<td>
						<img src='{{$template_root}}/images/edit_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href='admin.php?controller=admin_member&action=protect_group_edit&gid={{$gid}}'>{{$language.Edit}}</a>
						<img src='{{$template_root}}/images/delete_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href="admin.php?controller=admin_member&action=protect_lgroup_devgrp_delete&id={{$allpass[t].lgid}}" >{{$language.Delete}}</a>
					</tr>
					{{/section}}
					<tr>
						<td colspan="6" align="left">
							<input name="select_all" type="checkbox" onclick="javascript:for(var i=0;i<this.form.elements.length;i++){var e=this.form.elements[i];if(e.name=='id[]')e.checked=this.form.select_all.checked;}" value="checkbox">{{$language.Selectall}}&nbsp;&nbsp;<input type="submit"  value="{{$language.Deletedevices}}" onclick="my_confirm('{{$language.IPDeleteIPs}}');if(chk_form()) document.member_list.action='admin.php?controller=admin_member&action=protect_group_delete&type=lgroup-devgrp'; else return false;" class="an_02">
&nbsp;&nbsp;<input type="button"  value="{{$language.Add}}" onclick="javascript:location.href='admin.php?controller=admin_member&action=protect_group_edit&gid={{$gid}}'" class="an_02">						</td>
					</tr>
