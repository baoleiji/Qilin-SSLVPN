...
<ul>
  ...
  <tr>
    ...
    <FORM method=get name=f1   onSubmit="return false;"    action=admin.php>
         <INPUT id="controller " value=admin_user type=hidden name=c>
      <INPUT id=action value=index type=hidden   name=a> 
		      用户名:<INPUT name=username  size="12" class="wbk">
  ...
  <table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
	 <FORM method=post name=list     action=admin.php?controller=admin_user&amp;a=delete_all>
					
				<tr>
			 	     <TH width="3%" class="list_bg">选</TH>
			          <TH width="2%" class="list_bg">ID</TH>
			          <TH width="8%" class="list_bg">用户名</TH>
			          <TH width="5%" class="list_bg">权限</TH>
			          <TH width="10%" class="list_bg">邮箱</TH>
			          <TH width="16%" class="list_bg">描述</TH>
					   <TH class="list_bg"  width="5%"  >操作</TH>
					</tr>
					 {{section name=t loop=$all}}
  			 <TR class="list_tr_bg{{if $smarty.section.t.index % 2 ne 0}}1{{/if}}">
	    			  <TD><INPUT value={{$all[t].userid}} type=checkbox name=chk_member[]></TD>
			          <TD>{{$smarty.section.t.index+1}}</TD>
			          <TD>{{$all[t].username}}</TD>
			          <TD> 
			        	 {{if $all[t].level== 1}}管理员
						{{else }} 普通用户
						{{/if}}
			          </TD>
			          <TD>{{$all[t].email}}</TD>
			          <TD>{{$all[t].desc}}</TD>
			          <TD >
			          <input name="edit" type="button" value="编辑"  onclick="javascript:location.href='admin.php?controller=admin_user&action=edit&userid={{$all[t].userid}}'"  />
			          </TD>
					</tr>
					{{/section}}
		<TR>
          ...
          <INPUT  onclick="javascript:for(var i=0;i<this.form.elements.length;i++){var e=this.form.elements[i];if(e.name=='chk_member[]')e.checked=this.form.select_all.checked;}" 
            value=checkbox type=checkbox name=select_all>选本页显示的所有记录&nbsp;&nbsp;<INPUT class=an_02 onClick="my_confirm('删除所选记录');if(!chk_form())  return false;" value=批量删除所选记录 type=submit>      
             ...
             <input type="button" onclick="window.location='admin.php?controller=admin_user&action=add'"  value=" 添加 "  class="an_02">
