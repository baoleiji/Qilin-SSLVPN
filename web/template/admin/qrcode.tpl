...
<TABLE width="670" align="center" border="0" cellspacing="0" cellpadding="0">
  ...
  <TBODY>
  <TR>
    <TD align="center" valign="top"><img src="{{$template_root}}/images/mobile_top{{if $userinfo.usbkeystatus ne 11}}1{{/if}}.jpg" width="670" height="43"></TD>
  </TR>
  <TR>
...
<TR>
                    ...
                    <TR>
					  ...
					  {{else}}
					   {{if $userinfo.authtype}}
					  {{if $userinfo.localauth and $smarty.session.authtype ne 'localauth'}}
					   <TR>
                    ...
                    <TR>
					  ...
					  {{/if}}
					  {{if $userinfo.radiusauth and $smarty.session.authtype ne 'radiusauth'}}
                      ...
                      <TD><INPUT name="radiuspwd" id="ldapwd" style="width: 200px; height: 30px; line-height: 30px;" type="password"></TD></TR>
					   {{/if}}
					  {{if $userinfo.ldapauth and $smarty.session.authtype ne 'ldapauth'}}
                    <TR>
...
{{/section}} </SELECT></TD></TR>
                    <TR>
					  ...
					  {{/if}}
					   {{if $userinfo.adauth and $smarty.session.authtype ne 'adauth'}}
...
<INPUT class="phonebtn3" id="ElapsedTimeBu" onClick="sendsms(120);" type="button" value="获取短信密码"></TD></TR>
 {{/if}}
        ...
        <TR>
          <TD height="95" align="center">{{if $userinfo.usbkeystatus eq 11}}<a id="ios" href='#' onclick="openjpg('ios');return false;" >IOS下载</a>&nbsp;&nbsp;&nbsp;&nbsp;<a id="android" href='#' onclick="openjpg('android');return false;" >安卓下载</a>&nbsp;&nbsp;&nbsp;&nbsp;{{/if}}<INPUT name="button" {{if $userinfo.usbkeystatus eq 11}}onclick="return confirm('用户令牌将被新生成的手机令牌替换,确定操作吗？')"{{/if}} class="phonebtn1" id="button" onMouseOut="this.className='phonebtn1'" onMouseMove="this.className='phonebtn1cur'" type="submit" value="提 交">                      </TD></TR></TBODY></TABLE></TD></TR></TBODY></FORM></TABLE>
</DIV>
