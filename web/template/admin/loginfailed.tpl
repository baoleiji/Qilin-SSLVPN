...
<link href="{{$template_root}}/cssjs/dtree.css" rel="stylesheet" type="text/css" /><link type="text/css" rel="stylesheet" href="{{$template_root}}/cssjs/jscal2.css" />
...
function searchit(){
	var url = "admin.php?controller=admin_reports&action=loginfailed";
...
{{/if}}
{{if $smarty.session.ADMIN_LEVEL eq 1 or $smarty.session.ADMIN_LEVEL eq 2 or $smarty.session.ADMIN_LEVEL eq 21 or $smarty.session.ADMIN_LEVEL eq 3 or $smarty.session.ADMIN_LEVEL eq 101}}
	<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_reports&action=loginfailed">登录尝试</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li>
{{/if}}
...
//cal.manageFields("f_rangeEnd_trigger", "f_rangeEnd", "%Y-%m-%d %H:%M:%S");
  ...
  <tr><td><table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
					<tr>
						<th class="list_bg"   width="8%"><a href="admin.php?controller=admin_reports&action=loginfailed&orderby1=sourceip&orderby2={{$orderby2}}" >{{$language.SourceAddress}}</a></th>						
						<th class="list_bg"   width="8%"><a href="admin.php?controller=admin_reports&action=loginfailed&orderby1=serverip&orderby2={{$orderby2}}" >{{$language.Ipaddress}}</a></th>
						<th class="list_bg"   width="8%"><a href="admin.php?controller=admin_reports&action=loginfailed&orderby1=portocol&orderby2={{$orderby2}}" >登录协议</a></th>					
						<th class="list_bg"   width="8%"><a href="admin.php?controller=admin_reports&action=loginfailed&orderby1=audituser&orderby2={{$orderby2}}" >账号</a></th>
						<th class="list_bg"   width="8%"><a href="admin.php?controller=admin_reports&action=loginfailed&orderby1=ct&orderby2={{$orderby2}}" >次数</a></th>
						
					</tr>
					{{section name=t loop=$alllog}}
					<tr {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
	
						<td><a href="admin.php?controller=admin_reports&action=loginfailed&from={{$alllog[t].sourceip}}">{{$alllog[t].sourceip}}</a></td>
						<td><a href="admin.php?controller=admin_reports&action=loginfailed&serverip={{$alllog[t].serverip}}">{{$alllog[t].serverip}}</a></td>
						<td><a href="admin.php?controller=admin_reports&action=loginfailed&protocol={{$alllog[t].portocol}}">{{$alllog[t].portocol}}</a></td>
						<td><a href="admin.php?controller=admin_reports&action=loginfailed&audituser={{$alllog[t].audituser}}">{{$alllog[t].audituser}}</a></td>
						<td>{{$alllog[t].ct}}</td>
					</tr>
					{{/section}}
					<tr>
					...
					{{if $str}}
					<tr><td colspan="12" align="right"><img src="{{$template_root}}/images/excel.png" border=0>:{{$str}} </td></tr>
					<tr><td colspan="12" align="right"><img src="{{$template_root}}/images/html.png" border=0>:{{$strhtml}}</td></tr>
					<tr><td colspan="12" align="right"><img src="{{$template_root}}/images/word.png" border=0>:{{$strdoc}}</td></tr>
					{{/if}}
				</table>
	</td>
  </tr>
