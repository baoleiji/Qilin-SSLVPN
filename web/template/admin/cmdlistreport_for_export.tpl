	...
	<td class="main_content">
		<table bordercolor="white" cellspacing="1" cellpadding="5" border="0" width="100%"  class="BBtable"><tr>
					...
					<tr>
						...
						<th class="list_bg"  bgcolor="d9ecfa" width="10%">设备IP</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="10%">执行时间</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="10%">命令</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="10%">级别</th>
					</tr>
					{{section name=t loop=$reports}}
					<tr {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						<td>{{$smarty.section.t.index+1}}</td>
						<td width="10%">{{$reports[t].luser}}</td>
						<td width="10%">{{$reports[t].addr}}</td>
						<td width="10%">{{$reports[t].at}}</td>
						<td width="10%">{{$reports[t].cmd}}</td>
						<td width="10%">{{if !$reports[t].dangerlevel}}正常{{elseif $reports[t].dangerlevel eq 1}}危险{{elseif $reports[t].dangerlevel eq 2}}严重{{elseif $reports[t].dangerlevel eq 3}}警告{{elseif $reports[t].dangerlevel eq 4}}复核{{/if}}</td>
					</tr>
					{{/section}}
