...
{{if $smarty.session.ADMIN_LEVEL ne 3 and $smarty.session.ADMIN_LEVEL ne 21 and $smarty.session.ADMIN_LEVEL ne 101}}
	...
	{{/if}}
</ul><span class="back_img"><A href="admin.php?controller=admin_pro&action=sourceip&back=1"><IMG src="{{$template_root}}/images/back1.png" 
      width="80" height="30" border="0"></A></span>
</div></td></tr>
  <tr>
	<td class=""><table width="100%" border="0" cellspacing="0" cellpadding="0"  class="BBtable">
	<form name="ip_list" action="admin.php?controller=admin_pro&action=sourceiplist_delete&groupname={{$groupname}}" method="post">
                <TBODY>
				  
                  <TR>
				  <th class="list_bg" width="5%"><a href="admin.php?controller=admin_pro&action=sourceip_list&orderby1=sourceip&orderby2={{$orderby2}}" >选</a></TD>
                    <th class="list_bg" ><a href="admin.php?controller=admin_pro&action=sourceip_list&orderby1=sourceip&orderby2={{$orderby2}}" >Source IP</a></TD>
			...
			<td> {{$s[t].sourceip}}</td>	
				<td><img src='{{$template_root}}/images/edit_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href="admin.php?controller=admin_pro&action=sourceip_list_edit&groupname={{$s[t].groupname}}&sid={{$s[t]['sid']}}" >编辑</a>
				| <img src='{{$template_root}}/images/delete_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href="#" onClick="if(!confirm('{{$language.Delete_sure_}}？')) {return false;} else { location.href='admin.php?controller=admin_pro&action=sourceiplist_delete&sid={{$s[t]['sid']}}';}">{{$language.Delete}}</a>
				</td> 
			</tr>
			{{/section}}
	          <tr>
	          <td  colspan="2" align="left">
		          <input name="select_all" type="checkbox" onclick="javascript:for(var i=0;i<this.form.elements.length;i++){var e=this.form.elements[i];if(e.name=='chk_gid[]')e.checked=this.form.select_all.checked;}" value="checkbox">全选&nbsp;&nbsp;<input type="submit"  value="删除选中" onclick="my_confirm('删除所选IP');if(chk_form()) document.ip_list.action='admin.php?controller=admin_pro&action=sourceiplist_delete'; else return false;" class="an_02">&nbsp;&nbsp;&nbsp;&nbsp; 
					<input type="button" name="submit" onclick="location='admin.php?controller=admin_pro&action=sourceip_list_edit&groupname={{$groupname}}'" value=" 增加 " class="an_02" />
		   		</td>
