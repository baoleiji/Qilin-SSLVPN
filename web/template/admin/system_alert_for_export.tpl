...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
	<td class="main_content">
		...
		<tr>
					<td colspan="5">{{if $f_rangeStart ne '' or $f_rangeStart ne ''}}日期:{{$f_rangeStart}} 到 {{$f_rangeEnd}}{{/if}}</td>
					</tr>
					<tr>
						<th class="list_bg"  bgcolor="d9ecfa" width="10%">序号</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="10%">内容</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="10%">当前值</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="10%">时间</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="10%">发送邮件</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="10%">发送短信</th>
					</tr>
					{{section name=t loop=$reports}}
					<tr {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						<td width="10%">{{$smarty.section.t.index+1}}</td>
						<td width="30%">{{$reports[t].context}}</td>
						<td width="5%">{{$reports[t].value}}</td>
						<td width="10%">{{$reports[t].datetime}}</td>
						<td width="5%">{{if $reports[t].mail_status}}成功{{else}}失败{{/if}}</td>
						<td width="5%">{{if $reports[t].sms_status}}成功{{else}}失败{{/if}}</td>
					</tr>
					{{/section}}
