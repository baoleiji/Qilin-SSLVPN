...
<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_monitor&action=dns_monitor">DNS</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li>
</ul>
</div></td></tr>
<tr>
	<td align="left" colspan = "7"><table width="100%" border="0" cellspacing="0" cellpadding="0"  class="main_content">
      ...
      <tr  >
        <td class="" width="100%">
		  ...
		  <TABLE class=BBtable cellSpacing=0 cellPadding=0 width="100%"             align=center>
              ...
              <TR>
				...
				<TD width="9%" class="list_bg"><a href = "admin.php?controller=admin_monitor&action=oracle_disk_monitor&orderby1=disk_name&orderby2={{$orderby2}}">类型</a></TD>
				<TD width="9%" class="list_bg"><a href = "admin.php?controller=admin_monitor&action=oracle_disk_monitor&orderby1=disk_name&orderby2={{$orderby2}}">延迟(ms)</a></TD>
		...
		<TD class=""><a onclick="window.open ('admin.php?controller=admin_detail&ip={{$hosts[h].oracle_ip}}{{if $hosts[h].device_type|lower eq 'cisco' }}&action=ciscoindex{{/if}}', 'newwindow', 'top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=yes,location=no, status=no');return false;" href="#" target="_blank" >{{$hosts[h].device_ip}}</a></TD>
		<TD class="">{{$hosts[h].hostname}}</TD>
		<TD class="" style="text-align:left">{{$hosts[h].datetime}}</TD>
		<TD class="">{{if $hosts[h].type eq 1}}授权域可用性{{else}}非授权域可用性{{/if}}</TD>
		<TD class="">{{$hosts[h].delayvalue}}</TD>
		...
		<TD class="" style="text-align:left"><img src='{{$template_root}}/images/edit_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href='admin.php?controller=admin_monitor&action=dns_monitor_edit&id={{$hosts[h].id}}'>修改</a></TD>		
	  </TR>
	{{sectionelse}}
	   <TR>
