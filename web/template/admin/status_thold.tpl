...
<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_thold&action=status_thold">主机配置</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li>
        ...
        <td class="">
		  ...
		  <TABLE class=BBtable{{if $smarty.get.from eq 'hostview'}}1{{/if}} cellSpacing=0 cellPadding=0 width="100%"             align=center>
			  ...
			  <TD width="1%" class="list_bg{{if $smarty.get.from eq 'hostview'}}1{{/if}}"></td>
				{{if $smarty.get.from ne 'hostview'}}
                <TD width="9%" class="list_bg{{if $smarty.get.from eq 'hostview'}}1{{/if}}"><a href = "admin.php?controller=admin_thold&action=status_thold&orderby1=device_ip&orderby2={{$orderby2}}">主机</a></TD>
				{{/if}}
				<TD width="5%" class="list_bg{{if $smarty.get.from eq 'hostview'}}1{{/if}}"><a href = "admin.php?controller=admin_thold&action=status_thold&orderby1=type&orderby2={{$orderby2}}">类型</a></TD>
				<TD width="5%" class="list_bg{{if $smarty.get.from eq 'hostview'}}1{{/if}}"><a href = "admin.php?controller=admin_thold&action=status_thold&orderby1=disk&orderby2={{$orderby2}}">硬盘</a></TD>
                <TD width="8%"  class="list_bg{{if $smarty.get.from eq 'hostview'}}1{{/if}}"><a href = "admin.php?controller=admin_thold&action=status_thold&orderby1=value&orderby2={{$orderby2}}">当前值</a></TD>
                <TD width="8%"  class="list_bg{{if $smarty.get.from eq 'hostview'}}1{{/if}}"><a href = "admin.php?controller=admin_thold&action=status_thold&orderby1=enable&orderby2={{$orderby2}}">是否采集</a></TD>
                <TD width="8%"  class="list_bg{{if $smarty.get.from eq 'hostview'}}1{{/if}}"><a href = "admin.php?controller=admin_thold&action=status_thold&orderby1=lowvalue&orderby2={{$orderby2}}">最低值</a></TD>
                <TD width="8%"  class="list_bg{{if $smarty.get.from eq 'hostview'}}1{{/if}}"><a href = "admin.php?controller=admin_thold&action=status_thold&orderby1=highvalue&orderby2={{$orderby2}}">最大值</a></TD>
                <TD width="6%"  class="list_bg{{if $smarty.get.from eq 'hostview'}}1{{/if}}"><a href = "admin.php?controller=admin_thold&action=status_thold&orderby1=enable&orderby2={{$orderby2}}">邮件告警</a></TD>
                <TD width="6%"  class="list_bg{{if $smarty.get.from eq 'hostview'}}1{{/if}}"><a href = "admin.php?controller=admin_thold&action=status_thold&orderby1=enable&orderby2={{$orderby2}}">短信告警</a></TD>
				 <TD width="8%"  class="list_bg{{if $smarty.get.from eq 'hostview'}}1{{/if}}">操作</TD>
              </TR>
			<form action="" method="post" name="member_list">
			{{section name=g loop=$tholdlist}}
			<TR bgcolor="f7f7f7">
			<TD class="">{{if $smarty.session.ADMIN_LEVEL }}<input type="checkbox" name="chk_member[]" value="{{$tholdlist[g].seq}}">{{/if}}</TD>
			{{if $smarty.get.from ne 'hostview'}}
			...
			<TD class="" style="text-align:left">{{if $tholdlist[g].mail_alarm}}是{{else}}否{{/if}}</TD>
			<TD class="" style="text-align:left">{{if $tholdlist[g].sms_alarm}}是{{else}}否{{/if}}</TD>
			<TD class="" style="text-align:left">{{if $smarty.session.ADMIN_LEVEL }}<img src='{{$template_root}}/images/edit_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href='admin.php?controller=admin_thold&action=status_thold_edit&id={{$tholdlist[g].seq}}&from={{$smarty.get.from}}&ip={{$tholdlist[g].device_ip}}'>修改</a>
			&nbsp; | <img src="{{$template_root}}/images/scico.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_thold&action=status_thold_delete&id={{$tholdlist[g].seq}}&from={{$smarty.get.from}}&ip={{$ip}}">{{$language.Delete}}</a>{{/if}}</TD>
			</TR>
			<input type="hidden" name="seq[]" value="{{$tholdlist[g].seq}}" />
			{{/section}}
			<tr><td colspan="2">{{if $smarty.session.ADMIN_LEVEL }}<input name="select_all" type="checkbox" onClick="javascript:for(var i=0;i<this.form.elements.length;i++){var e=document.member_list.elements[i];if(e.name=='chk_member[]')e.checked=document.member_list.select_all.checked;}" value="checkbox">&nbsp;&nbsp;<input type="submit"  value="删除" onClick="if(confirm('确定删除?')){document.member_list.action='admin.php?controller=admin_thold&action=status_thold_delete&from={{$smarty.get.from}}&ip={{$ip}}'; }" class="an_02">&nbsp;&nbsp;{{/if}}</td><td colspan="7" align="right">{{if $smarty.session.ADMIN_LEVEL }}<input type="submit" name="edit" value="修改" class="an_02">&nbsp&nbsp;&nbsp;{{/if}}共{{$total}}个记录  {{$page_list}}  页次：{{$curr_page}}/{{$total_page}}页  {{$items_per_page}}个记录/页  转到第<input name="pagenum" type="text" class="wbk" size="2" onKeyPress="if(event.keyCode==13) window.location='admin.php?controller=admin_thold&action=status_thold&page='+this.value;">页&nbsp;&nbsp;&nbsp;{{if $smarty.session.ADMIN_LEVEL eq 3}}<a href="{{$curr_url}}&derive=1" target="hide">{{$language.ExportresulttoExcel}}</a>{{/if}}</td></tr>
			</form>
			</TBODY>
		</TABLE>
