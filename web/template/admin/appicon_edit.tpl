...
<ul>
	...
	{{/if}}
</ul><span class="back_img"><A href="admin.php?controller=admin_config&action=appicon_list&back=1"><IMG src="{{$template_root}}/images/back1.png" 
  ...
  <tr>
	<td class=""><table width="100%" border="0" cellspacing="0" cellpadding="0" >
          <tr>
            <td align="center">
    <form name="f1" method=post action="admin.php?controller=admin_config&action=appicon_save&id={{$p.id}}" enctype="multipart/form-data">
	...
	{{assign var="trnumber" value=0}}
	   ...
	   {{assign var="trnumber" value=$trnumber+1}}
		...
		<td width="67%">{{if $p.path}}<img src="upload/{{$p.path}}" /><br>{{/if}}<input name="path" type="file" ></td>
	  </tr>
