...
{{if $smarty.session.ADMIN_LEVEL ne 3 and $smarty.session.ADMIN_LEVEL ne 21 and $smarty.session.ADMIN_LEVEL ne 101}}
	...
	{{/if}}
	<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_workflow&action=workflow_contant">申请描述</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li>
  ...
  <tr>
	...
	<form name="ip_list" action="admin.php?controller=admin_workflow&action=workflow_contant_delete" method="post">
                <TBODY>
                  <TR>
			
                    <th class="list_bg" width="5%"><a href="admin.php?controller=admin_workflow&action=workflow_contant&orderby1=groupname&orderby2={{$orderby2}}" >选</a></TD>
                    <th class="list_bg"  width="20%"><a href="admin.php?controller=admin_workflow&action=workflow_contant&orderby1=groupname&orderby2={{$orderby2}}" >名称</a></TD>
                    <th class="list_bg"  width="30%"><a href="admin.php?controller=admin_workflow&action=workflow_contant&orderby1=groupname&orderby2={{$orderby2}}" >描述</a></TD>
			...
			<tr  {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
				<td width="5%">{{if $s[t].status}}<input type="checkbox" name="chk_gid[]" value="{{$s[t]['sid']}}">{{/if}}</td>
				 <td> {{$s[t].name}}</td>
				  <td> {{$s[t].desc}}</td>
				<td style="TEXT-ALIGN: left;">{{if $s[t].status}}<img src='{{$template_root}}/images/edit_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href="admin.php?controller=admin_workflow&action=workflow_contant_edit&sid={{$s[t]['sid']}}&sid={{$s[t]['sid']}}" >编辑</a>{{/if}}
				</td> 
			</tr>
			{{/section}}
	          <tr>
	           <td  colspan="2" align="left">
		          <input name="select_all" type="checkbox" onclick="javascript:for(var i=0;i<this.form.elements.length;i++){var e=this.form.elements[i];if(e.name=='chk_gid[]')e.checked=this.form.select_all.checked;}" value="checkbox">全选&nbsp;&nbsp;<input type="submit"  value="删除选中" onclick="my_confirm('删除所选');if(chk_form()) document.ip_list.action='admin.php?controller=admin_workflow&action=workflow_contant_delete'; else return false;" class="an_02">&nbsp;&nbsp;&nbsp;&nbsp; 
					<input type="button" name="submit" onclick="location='admin.php?controller=admin_workflow&action=workflow_contant_edit'" value=" 增加 " class="an_02" />
		   		</td>
