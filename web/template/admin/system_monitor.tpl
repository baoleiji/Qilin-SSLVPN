...
function searchit(){
	document.f1.elements.action = "admin.php?controller=admin_monitor&action=system_monitor";
	document.f1.elements.action += "&ip="+document.f1.elements.ip.value;
	{{if $_config.LDAP}}
	document.f1.elements.action += "&ldapid1="+document.f1.elements.ldapid1.value;
	document.f1.elements.action += "&ldapid2="+document.f1.elements.ldapid2.value;
	{{/if}}
	document.f1.elements.action += "&g_id="+document.f1.elements.g_id.value;
	document.location=document.f1.elements.action;
	return true;
}
...
<tr>
	<td align="left" colspan = "7"><table width="100%" border="0" cellspacing="0" cellpadding="0"  class="main_content">
      ...
      <tr  >
        <td class="">
		  ...
		  <TABLE class=BBtable cellSpacing=0 cellPadding=0 width="100%"             align=center>
              ...
              <TR>
				...
				<TD width="9%" class="list_bg"><a href = "admin.php?controller=admin_monitor&action=system_monitor&orderby1=b.device_type&orderby2={{$orderby2}}">操作系统</a></TD>
                <TD width="9%"  class="list_bg"><a href = "admin.php?controller=admin_monitor&action=system_monitor&orderby1=cpu&orderby2={{$orderby2}}">CPU</a></TD>
                <TD width="9%"  class="list_bg"><a href = "admin.php?controller=admin_monitor&action=system_monitor&orderby1=memory&orderby2={{$orderby2}}">内存</a></TD>
                <TD width="9%"  class="list_bg"><a href = "admin.php?controller=admin_monitor&action=system_monitor&orderby1=swap&orderby2={{$orderby2}}">交换分区</a></TD>
                <TD width=""  class="list_bg"><a href = "admin.php?controller=admin_monitor&action=system_monitor&orderby1=disk&orderby2={{$orderby2}}">存储</a></TD>
              </TR>
		...
		<TD class="">{{if $hosts[h].monitor eq 1}}SNMP{{elseif $hosts[h].monitor eq 2}}登录{{elseif $hosts[h].monitor eq 3}}上传{{else}}关闭{{/if}}</TD>
