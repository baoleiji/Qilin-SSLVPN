...
<ul>
	...
	{{/if}}
</ul><span class="back_img"><A href="admin.php?controller=admin_config&action=appserver_list&back=1"><IMG src="{{$template_root}}/images/back1.png" 
  ...
  <tr>
	...
	{{assign var="trnumber" value=0}}
		...
		{{section name=s loop=$pserver}}
		<option value="{{$pserver[s].device_ip}}" {{if $p.appserverip eq $pserver[s].device_ip}}selected{{/if}}>{{$pserver[s].device_ip}}</option>
		{{/section}}		
		</select>
		</td>
	  </tr>
	  {{assign var="trnumber" value=$trnumber+1}}
		...
		<td width="67%"><textarea name="description" rows=10 cols=50>{{$p.description}}</textarea></td>
	  </tr>
...
<input type=submit  value="{{$language.Save}}" class="an_02"></td></tr>
	</table>
<br>
