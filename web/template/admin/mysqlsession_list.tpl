...
function searchit(){
	document.search.action = "admin.php?controller=admin_mysql";
...
<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_mysql">MySQL</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li>
</ul>
</div></td></tr>
    <tr>
    <td >
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="main_content"><form action="admin.php?controller=admin_mysql" method="post" name="search" >
  <tr>
    <td></td>
    <td>
...
cal.manageFields("f_rangeEnd_trigger", "f_rangeEnd", "%Y-%m-%d %H:%M:%S");
  ...
  <tr><td><table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
					<tr>
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_mysql&orderby1=d_addr&orderby2={{$orderby2}}">目的地址</a></th>
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_mysql&orderby1=user&orderby2={{$orderby2}}">登录用户名</a></th>
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_mysql&orderby1=start&orderby2={{$orderby2}}">开始时间</a></th>
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_mysql&orderby1=end&orderby2={{$orderby2}}">结束时间</a></th>
					...
					<tr {{if $allsession[t].dangerous > 5}}bgcolor="red"{{elseif $allsession[t].dangerous > 0}}bgcolor="yellow" {{elseif $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						<td><a href="admin.php?controller=admin_mysql&d_addr={{$allsession[t].d_addr}}">{{$allsession[t].d_addr}}</a></td>
						<td><a href="admin.php?controller=admin_mysql&user={{$allsession[t].user}}">{{$allsession[t].user}}</a></td>
						<td>{{$allsession[t].start}}</ td>
						<td>{{$allsession[t].end}}</td>						
						<td style="TEXT-ALIGN: left;"><img src="{{$template_root}}/images/ico2.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_mysql&action=view&sid={{$allsession[t]['sid']}}&command={{$command}}">查看</a>{{if $admin_level == 2}} | <img src="{{$template_root}}/images/scico.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_mysql&action=del_session&sid={{$allsession[t]['sid']}}">删除</a>{{/if}}</td>
					</tr>
					{{/section}}
					<tr>
