...
function searchit(){
	var url = "admin.php?controller=admin_reports&action=cmdlistreport_users";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <TD height="33" class="main_content" colspan="3"><FORM name="f1" action="admin.php?controller=admin_reports&amp;action=cmdlistreport_users" 
            method="post">
        资源组：{{if $_config.LDAP}}
      ...
      <FORM name="f2" onSubmit="javascript:saveAccount=false;" action="admin.php?controller=admin_reports&amp;action=docmdlistreport_users" 
  ...
  <tr>
    <TD height="33" colspan="3"><TABLE width="100%" align="center" class="BBtable" bgcolor="#ffffff" 
      ...
      <TBODY>
      <TR>
        <TD width="10%" align="right">绑定用户 </TD>
        <TD><TABLE width="100%">
            <TBODY>
              <TR>
			  {{section name=u loop=$users}}
                <TD width="180"><INPUT name="Check[]" type="checkbox" {{$users[u].check}} value="{{$users[u].username}}">{{$users[u].username}}({{$users[u].realname}})</TD>
