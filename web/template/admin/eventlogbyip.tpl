...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#F1F1F1">
  ...
  <table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable1">
	 <FORM method=post name=list     action=admin.php?controller=admin_eventlogs&amp;a=delete_all&amp;t=eventlogs>
					
				<tr>
			 	     <TH width="3%" class="list_bg1">选</TH>
			          <TH width="3%" class="list_bg1">ID</TH>
			           <TH width="8%" class="list_bg1">程序</TH>
			          <TH width="5%" class="list_bg1">级别</TH>
			          <TH width="10%" class="list_bg1">时间</TH>
			            <TH width="15%" class="list_bg1">事件</TH>
			          <TH width="5%" class="list_bg1">状态</TH>

					      <TH class="list_bg1"  width="5%"  >操作</TH>
					</tr>
					 {{section name=t loop=$all}}
  			 <TR class="list_tr_bg{{if $smarty.section.t.index % 2 ne 0}}1{{/if}}">
	    			  <TD>{{if $smarty.session.ADMIN_LEVEL }}<INPUT value={{$all[t].seq}} type=checkbox name=chk_member[]>{{/if}}</TD>
					...
					{{/section}}
		<TR>
          <TD colSpan=9 align=left>{{if $smarty.session.ADMIN_LEVEL }}
