	...
	<form name="member_list" action="admin.php?controller=admin_member&action=protect_group_delete" method="post">
				<table bordercolor="white" cellspacing="1" cellpadding="5" border="0" width="100%"  class="BBtable">
					<tr>
						...
						<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_member&action=protect_group&gid={{$id}}&type=group&orderby1=device_ip&orderby2={{$orderby2}}" >{{$language.Host}}IP</a></th>
						<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_member&action=protect_group&gid={{$id}}&type=group&orderby1=hostname&orderby2={{$orderby2}}" >{{$language.HostName}}</a></th>
						<th class="list_bg"  width="15%"><a href="admin.php?controller=admin_member&action=protect_group&gid={{$id}}&type=group&orderby1=username&orderby2={{$orderby2}}" >{{$language.LocalUser}}{{$language.User}}</a></th>
						<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_member&action=protect_group&gid={{$id}}&type=group&orderby1=login_method&orderby2={{$orderby2}}" >{{$language.Loginmode}}</a></th>
						<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_member&action=protect_group&gid={{$id}}&type=group&orderby1=forbidden_commands_groups&orderby2={{$orderby2}}" >{{$language.Black}}</a></th>
						<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_member&action=protect_group&gid={{$id}}&type=group&orderby1=weektime&orderby2={{$orderby2}}" >{{$language.WeekTimepolicy}}</a></th>
						<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_member&action=protect_group&gid={{$id}}&type=group&orderby1=sourceip&orderby2={{$orderby2}}" >{{$language.SourceIP}}</a></th>
					...
					<tr {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						...
						<td><a href="admin.php?controller=admin_pro&action=dev_index&ip={{$s_dev[t].device_ip}}">{{$allpass[t].device_ip}}</a></td>
...
&nbsp;&nbsp;<input type="button"  value="{{$language.Add}}" onclick="javascript:f1table.style.display=''" class="an_02">						</td>
	...
	<form name="f1" method=post OnSubmit='return check()' action="admin.php?controller=admin_member&action=protect_groupgrp&uid={{$id}}&type=group">
		<tr bgcolor="f7f7f7">
	...
	<tr>
		...
		<td align=right>
			<input type="radio" name="controller" value="{{$alldevgroup[g].id}}" onClick="location.href='admin.php?controller=admin_member&action=protect_dev&g_id={{$alldevgroup[g].id}}&uid={{$id}}&type=group'">
		</td>
		<td>
