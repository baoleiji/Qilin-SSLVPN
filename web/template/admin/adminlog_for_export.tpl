...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0" >
        <tr >
          ...
          <td height="35" bgcolor="#528AC7" class="main_title">{{$language.ReportManager}}——变更报表</td>         
  ...
  <tr>
	<td class="main_content">
		...
		<tr>
					<td colspan="7">日期:{{$f_rangeStart}} 到 {{$f_rangeEnd}}</td>
					</tr>
					<tr>
						 <th class="list_bg" >序号</TD>
                    <th class="list_bg" >管理员</TD>
                    <th class="list_bg" >操作</TD>
					<th class="list_bg" >操作对象</TD>
					<th class="list_bg" >结果</TD>
					<th class="list_bg" >操作时间</TD>
					</tr>
					{{section name=t loop=$alldev}}
					<tr {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						<td>{{$smarty.section.t.index+1}}</td>
						<td>{{$alldev[t].administrator}}</td>
						<td>{{$alldev[t].action}}</td>
						<td>{{if $alldev[t].type eq 1}}系统用户组：{{$alldev[t].resource}},运维用户:{{$alldev[t].luser}}
							{{elseif $alldev[t].type eq 2}}系统用户组：{{$alldev[t].resource}},资源组:{{$alldev[t].lgroup}}
							{{elseif $alldev[t].type eq 3}}应用用户组：{{$alldev[t].resource}},运维用户:{{$alldev[t].luser}}
							{{elseif $alldev[t].type eq 4}}应用用户组：{{$alldev[t].resource}},资源组:{{$alldev[t].lgroup}}
							{{elseif $alldev[t].type eq 5}}设备：{{$alldev[t].resource}}，设备用户：{{$alldev[t].resource_user}},运维用户:{{$alldev[t].luser}}
							{{elseif $alldev[t].type eq 6}}设备：{{$alldev[t].resource}}，设备用户：{{$alldev[t].resource_user}},资源组:{{$alldev[t].lgroup}}
							{{elseif $alldev[t].type eq 7}}应用名称：{{$alldev[t].resource}},运维用户:{{$alldev[t].luser}}
							{{elseif $alldev[t].type eq 8}}应用名称：{{$alldev[t].resource}},资源组:{{$alldev[t].lgroup}}
							{{elseif $alldev[t].type eq 11}}运维用户:{{$alldev[t].luser}}
							{{elseif $alldev[t].type eq 12}}资源组:{{$alldev[t].lgroup}}
							{{elseif $alldev[t].type eq 13}}设备：{{$alldev[t].resource}}，设备用户：{{$alldev[t].resource_user}}
							{{elseif $alldev[t].type eq 14}}设备：{{$alldev[t].resource}}
							{{elseif $alldev[t].type eq 15}}日志类型:{{$alldev[t].resource}}
							
							{{/if}}
						</td>
						<td>{{$alldev[t].result}}</td>
						<td>{{$alldev[t].optime}}</td>
					</tr>
					{{/section}}
