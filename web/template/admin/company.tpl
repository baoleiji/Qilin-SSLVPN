...
<li class="me_b"><img src="{{$template_root}}/images/an11.jpg" align="absmiddle"/><a href="admin.php?controller=admin_host&action=system">操作系统</a><img src="{{$template_root}}/images/an33.jpg" align="absmiddle"/></li> 
  ...
  <tr>
    ...
    <FORM method=get name=f1   onSubmit="return false;"    action=admin.php>
      ...
      <INPUT id=action value=index type=hidden   name=a> 
		     公司名称:<INPUT name=company  size="12" class="wbk">
		      <INPUT  size="12" class="bnnew2"  onClick="JavaScript: document.getElementById('action').value='company';submit();" value=查找  type=button> 
	</FORM>
</td>
  </tr>
</table>	
  ...
  <table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
	 <FORM method=post name=list     action=admin.php?controller=admin_host&amp;a=delete_all&amp;t=company>
					
				<tr>
			 	       <TH width="3%" class="list_bg">选</TH>
			          <TH width="3%" class="list_bg">ID</TH>
			          <TH width="10%" class="list_bg">公司名称</TH>
			          <TH width="10%" class="list_bg">电话</TH>
			          <TH width="10%" class="list_bg">地址</TH>
			          <TH width="6%" class="list_bg">联系人</TH>
					 ...
					 {{section name=t loop=$all}}
			          ...
			          <TD>{{$all[t].company}}</TD>
			          <TD>{{$all[t].telephone}}</TD>
			          <TD>{{$all[t].address}}</TD>
			          <TD>{{$all[t].connecter}}</TD>
					  <TD>{{$all[t].desc}}</TD>
					   <TD >
			          <input name="edit" type="button" value="编辑"  onclick="javascript:location.href='admin.php?controller=admin_host&action=edit&t=company&id={{$all[t].id}}'"  />
			          </TD>
					</tr>
					{{/section}}
		<TR>
         ...
         <input type="button" onclick="window.location='admin.php?controller=admin_host&action=edit&t=company'"  value=" 添加 "  class="an_02">
