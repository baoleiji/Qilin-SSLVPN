...
<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_host&action=system">操作系统</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li> 
  ...
  <tr>
    ...
    <FORM method=get name=f1   onSubmit="return false;"    action=admin.php>
       <INPUT id="controller " value=admin_host type=hidden name=c>
      <INPUT id=action value=index type=hidden   name=a> 
		     操作系统名称:<INPUT name=system  size="12" class="wbk"> 
		      <INPUT  size="12" class="bnnew2"  onClick="JavaScript: document.getElementById('action').value='system';submit();" value=查找  type=button> 
	</FORM>
</td>
  </tr>
</table>	
  ...
  <table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
	 <FORM method=post name=list     action=admin.php?controller=admin_host&amp;a=delete_all&amp;t=system_template>
					
				<tr>
			 	    <TH width="3%" class="list_bg">选</TH>
			          <TH width="3%" class="list_bg">ID</TH>
			          <TH width="10%" class="list_bg">操作系统名称</TH>
			          <TH width="10%" class="list_bg">描述</TH>
			           
					<TH class="list_bg"  width="5%"  >修改</TH>
					<TH width="10%" class="list_bg">&nbsp;</TH>
					</tr>
					 {{section name=t loop=$all}}
			          ...
			          <TD>{{$all[t].desc}}</TD>
			 			...
			 			<TD >
			          <input name="edit" type="button" value="编辑"  onclick="javascript:location.href='admin.php?controller=admin_host&action=edit&t=system_template&id={{$all[t].id}}'"  />
			          </TD>
			             <TD>&nbsp;</TD>
					</tr>
					{{/section}}
		<TR>
         ...
         <input type="button" onclick="window.location='admin.php?controller=admin_host&action=edit&t=system_template'"  value=" 添加 "  class="an_02">
