	...
	<td align="left" colspan = "7"><table width="100%" border="0" cellspacing="0" cellpadding="0"  class="main_content">
		...
		<TBODY>
		 <TR>
			...
			<select name='t' onchange='javascript:window.location="admin.php?controller=admin_monitor&action="+this.value'>
			<option value="oracle_tablespace_monitor" >表空间</option>
			<option value="oracle_disk_monitor" >磁盘</option>
			<option value="oracle_diskgroup_monitor" selected>磁盘组</option>
			</select>
      ...
      <tr  >
        <td class="" width="100%">
		  ...
		  <TABLE class=BBtable cellSpacing=0 cellPadding=0 width="100%"             align=center>
              ...
              <TR>
                <TD width="9%" class="list_bg"><a href = "admin.php?controller=admin_monitor&action=oracle_diskgroup_monitor&orderby1=device_ip&orderby2={{$orderby2}}">设备IP</a></TD>
				<TD width="9%" class="list_bg"><a href = "admin.php?controller=admin_monitor&action=oracle_diskgroup_monitor&orderby1=hostname&orderby2={{$orderby2}}">主机名称</a></TD>
                <TD width="10%"  class="list_bg"><a href = "admin.php?controller=admin_monitor&action=oracle_diskgroup_monitor&orderby1=datetime&orderby2={{$orderby2}}">抓取时间</a></TD>
				<TD width="6%" class="list_bg"><a href = "admin.php?controller=admin_monitor&action=oracle_diskgroup_monitor&orderby1=disk_name&orderby2={{$orderby2}}">磁盘组编号</a></TD>
                <TD width="6%"  class="list_bg"><a href = "admin.php?controller=admin_monitor&action=oracle_diskgroup_monitor&orderby1=disk_path&orderby2={{$orderby2}}">磁盘组名称</a></TD>
                <TD width="6%"  class="list_bg"><a href = "admin.php?controller=admin_monitor&action=oracle_diskgroup_monitor&orderby1=total_size&orderby2={{$orderby2}}">总空间</a></TD>
                <TD width="6%"  class="list_bg"><a href = "admin.php?controller=admin_monitor&action=oracle_diskgroup_monitor&orderby1=free_size&orderby2={{$orderby2}}">剩余空间</a></TD>
                <TD width="6%"  class="list_bg"><a href = "admin.php?controller=admin_monitor&action=oracle_diskgroup_monitor&orderby1=mail_alarm&orderby2={{$orderby2}}">邮件告警</a></TD>
                <TD width="6%"  class="list_bg"><a href = "admin.php?controller=admin_monitor&action=oracle_diskgroup_monitor&orderby1=sms_alarm&orderby2={{$orderby2}}">短信告警</a></TD>
                <TD width="9%"  class="list_bg"><a href = "#">操作</a></TD>
              </TR>
			{{section name=h loop=$hosts}}
		 <TR {{if $smarty.section.h.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
		<TD class=""><a onclick="window.open ('admin.php?controller=admin_detail&ip={{$hosts[h].oracle_ip}}{{if $hosts[h].device_type|lower eq 'cisco' }}&action=ciscoindex{{/if}}', 'newwindow', 'top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=yes,location=no, status=no');return false;" href="#" target="_blank" >{{$hosts[h].oracle_ip}}</a></TD>
		<TD class="">{{$hosts[h].hostname}}</TD>
		<TD class="" style="text-align:left">{{$hosts[h].datetime}}</TD>
		<TD class="">{{$hosts[h].disk_name}}</TD>
		<TD class="" style="text-align:left">{{$hosts[h].disk_path}}</TD>
		<TD class="" style="text-align:left">{{$hosts[h].total_size}}</TD>
		<TD class="" style="text-align:left">{{$hosts[h].free_size}}</TD>
		...
		<TD class="" style="text-align:left"><img src='{{$template_root}}/images/edit_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href='admin.php?controller=admin_monitor&action=oracle_disk_monitor_edit&id={{$hosts[h].id}}'>修改</a></TD>		
	  </TR>
	{{sectionelse}}
	   <TR>
