...
<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_searchlogs">搜索条件</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li>
  ...
  <tr>
    ...
    <INPUT id="controller " value=admin_searchlogs type=hidden name=c>
      <INPUT id=action value=index type=hidden   name=a> 	
		      名称:<INPUT name=hostname  size="12" class="wbk"> 
  ...
  <table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
	 <FORM method=post name=list     action=admin.php?controller=admin_searchlogs&amp;a=delete_all>
					
				<tr>
			 	     <TH width="3%" class="list_bg">选</TH>
			          <TH width="3%" class="list_bg">ID</TH>
			          <TH width="8%" class="list_bg">名称</TH>
			          <TH width="5%" class="list_bg">状态</TH>
			          <TH width="10%" class="list_bg">启动时间</TH>
			          <TH width="10%" class="list_bg">结束时间</TH>
			           <TH width="15%" class="list_bg">sql</TH>
						<TH width="3%" class="list_bg">操作</TH>
					</tr>
  			 ...
  			 <TR class="list_tr_bg{{if $smarty.section.t.index % 2 ne 0}}1{{/if}}">
			          ...
			          <TD>{{$all[t].name}}</TD>
			          <TD> 
			         	 {{if $all[t].status == "1"}}结束
						{{else}}等待
						{{/if}}
			          
			          </TD>
			          <TD>{{$all[t].starttime}}</TD>
 					<TD>{{$all[t].endtime}}</TD>
 					<TD>{{$all[t].sql}}</TD>
 					 <TD >
			          <input name="show" type="button" value="查看"  onclick="window.location='admin.php?controller=admin_searchlogs&action=queryTable&seq={{$all[t].seq}}'"  />
			          </TD>
					</tr>
					{{/section}}
		<TR>
          ...
          <INPUT  onclick="javascript:for(var i=0;i<this.form.elements.length;i++){var e=this.form.elements[i];if(e.name=='chk_member[]')e.checked=this.form.select_all.checked;}" 
            value=checkbox type=checkbox name=select_all>选本页显示的所有记录&nbsp;&nbsp;<INPUT class=an_06 onClick="my_confirm('删除所选记录');if(!chk_form())  return false;" value=批量删除所选记录 type=submit>      
             ...
             <input type="button" onclick="window.location='admin.php?controller=admin_searchlogs&action=edit'"  value=" 添加 "  class="an_02">
