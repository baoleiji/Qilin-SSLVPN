...
<script>
function searchit(){
	var url = "admin.php?controller=admin_app&action=appresourcegroup_bind&id={{$id}}";
...
</ul><span class="back_img"><A href="admin.php?controller={{if $fromapp eq 'search'}}admin_pro&action=app_priority_search{{else}}admin_app&action=app_group{{/if}}&back=1"><IMG src="{{$template_root}}/images/back1.png" 
      width="80" height="30" border="0"></A></span>
</div></td></tr>
  <tr>
	<td class=""><table width="100%" border="0" cellspacing="0" cellpadding="0" >
          <tr>
            <td align="center"><form name="f1" method=post action="admin.php?controller=admin_app&action=appresourcegroup_bindsave&id={{$id}}">
	...
	&nbsp;&nbsp;<input  type="button" value=" 搜索 " onclick="searchit();" class="bnnew2">
	  ...
	  <tr><td align="right">组名</td><td>{{$ginfo.appgroupname}}</td></tr>
	  <tr><td align="right">已绑定资源数</td><td>{{$ginfo.devicesct}}</td></tr>
