...
<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_thold&action=snmp_interface_log">流量告警</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li>
      ...
      <tr  >
        <td class="">
		  ...
		  <TABLE class=BBtable{{if $smarty.get.from eq 'hostview'}}1{{/if}} cellSpacing=0 cellPadding=0 width="100%"             align=center>
              <TBODY>
              <TR>
                <TD width="10%" class="list_bg{{if $smarty.get.from eq 'hostview'}}1{{/if}}"><a href = "admin.php?controller=admin_thold&action=snmp_interface_log&orderby1=device_ip&orderby2={{$orderby2}}&from={{$smarty.get.from}}&ip={{$smarty.get.ip}}">设备IP</a></TD>
				<TD width="20%" class="list_bg{{if $smarty.get.from eq 'hostview'}}1{{/if}}"><a href = "admin.php?controller=admin_thold&action=snmp_interface_log&orderby1=datetime&orderby2={{$orderby2}}&from={{$smarty.get.from}}&ip={{$smarty.get.ip}}">时间</a></TD>
                <TD width="10%"  class="list_bg{{if $smarty.get.from eq 'hostview'}}1{{/if}}"><a href = "admin.php?controller=admin_thold&action=snmp_interface_log&orderby1=port_index&orderby2={{$orderby2}}&from={{$smarty.get.from}}&ip={{$smarty.get.ip}}">端口号</a></TD>
                <TD width=""  class="list_bg{{if $smarty.get.from eq 'hostview'}}1{{/if}}"><a href = "admin.php?controller=admin_thold&action=snmp_interface_log&orderby1=context&orderby2={{$orderby2}}&from={{$smarty.get.from}}&ip={{$smarty.get.ip}}">告警内容</a></TD>
              </TR>
			...
			<TD class="" style="text-align:left">{{$tholdlist[g].port_index}}</TD>
