...
<li class="me_b"><img src="{{$template_root}}/images/an11.jpg" align="absmiddle"/><a href="admin.php?controller=admin_relation&action=relationids">报警设备</a><img src="{{$template_root}}/images/an33.jpg" align="absmiddle"/></li> 
  ...
  <tr>
    ...
    <FORM method=get name=f1   onSubmit="return false;"    action=admin.php>
         <INPUT id="controller " value=admin_relation type=hidden name=c>
  ...
  <table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
	 <FORM method=post name=list     action=admin.php?controller=admin_relation&amp;a=delete_all&amp;t=relation>
					
				<tr>
			 	     <TH width="3%" class="list_bg">选</TH>
			          <TH width="5%" class="list_bg">ID</TH>
			          <TH width="10%" class="list_bg">报警设备</TH>
			          <TH width="10%" class="list_bg">关联字符</TH>
			          <TH width="10%" class="list_bg">服务器匹配</TH>
			          <TH width="6%" class="list_bg">服务器网段</TH>
			          <TH width="10%" class="list_bg">关联字符</TH>
  			 ...
  			 <TR class="list_tr_bg{{if $smarty.section.t.index % 2 ne 0}}1{{/if}}">
			          ...
			          <TD>{{$all[t].idsip}}</TD>
			          <TD>{{$all[t].idsmsg}}</TD>
			          <TD>{{$all[t].idsserverip}}</TD>
			          <TD>{{$all[t].devicesip}}</TD>
					  <TD>{{$all[t].devicesmsg}}</TD>
			          <TD>{{$all[t].level}}</TD>

			          <TD >
			          <input name="edit" type="button" value="编辑"  onclick="javascript:location.href='admin.php?controller=admin_relation&action=edit&t=relation&id={{$all[t].seq}}'"  />
			          </TD>
					</tr>
					{{/section}}
		<TR>
          ...
          <INPUT  onclick="javascript:for(var i=0;i<this.form.elements.length;i++){var e=this.form.elements[i];if(e.name=='chk_member[]')e.checked=this.form.select_all.checked;}" 
            value=checkbox type=checkbox name=select_all>选本页显示的所有记录&nbsp;&nbsp;<INPUT class=an_02 onClick="my_confirm('删除所选记录');if(!chk_form())  return false;" value=批量删除所选记录 type=submit>      
             ...
             <input type="button" onclick="window.location='admin.php?controller=admin_relation&action=edit&t=relation'"  value=" 添加 "  class="an_02">
         <input name="export" type="button" value="导入记录"  onclick="javascript:location.href='admin.php?controller=admin_relation&action=import'" class="an_02" />
         <input name="export" type="button" value="导出记录"  onclick="javascript:location.href='admin.php?controller=admin_relation&action=export'" class="an_02" />
