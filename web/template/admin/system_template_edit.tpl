...
<td   width=100%>
    	<FORM method=post name=edit 
            action=admin.php?controller=admin_host&action=save&t=system_template>
           <input type='hidden' name='id' value="{{$result.id}}">
      
	<table width=100%  class="BBtable">
	{{assign var="trnumber" value=0}}
	<tr {{if $trnumber % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
                <TD align="right" class=main_list_td1>操作系统名称：</TD>
                    <TD align="left" class=main_list_td1>
                   			<input   size=20 type="text" name="system"   value="{{$result.system}}"/>
                      </TD>
	</tr>
