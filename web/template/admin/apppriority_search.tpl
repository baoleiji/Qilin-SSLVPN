...
<input type="hidden" name="type" value="luser" />
				<table bordercolor="white" cellspacing="0" cellpadding="0" border="0" width="100%"  class="BBtable">
				 <tr>
    <th class="list_bg" colspan="2">{{$language.Man}}：{{$language.Search}}应用权限,留空表示{{$language.no}}限制 </th>
  </tr>
					...
					<tr  {{if $trnumber++ % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						...
						{{section name=a loop=$appservers}}
						<option value="{{$appservers[a].appserverip}}">{{$appservers[a].appserverip}}</option>
						{{/section}}
						</select></td>
					</tr>
					<tr  {{if $trnumber++ % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
