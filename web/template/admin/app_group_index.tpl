...
function searchit(){
	...
	{{/if}}
	{{/if}}
	document.f1.action = "admin.php?controller=admin_app&action=app_group";
...
<ul>
	...
	<td class="" colspan = "4"><table width="100%" border="0" cellspacing="0" cellpadding="0"  class="main_content">
                ...
                <TBODY>
				 <TR>
                    <TD >
					<form name ='f1' action='admin.php?controller=admin_app&action=app_group' method=post>
  ...
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="BBtable">
                <TBODY>
                  ...
                  <TR>
                    <th class="list_bg" width="40%"><a href="admin.php?controller=admin_app&action=app_group&orderby1=appgroupname&orderby2={{$orderby2}}" >组名</a></TD>
                    <th class="list_bg" width="40%"><a href="admin.php?controller=admin_app&action=app_group&orderby1=desc&orderby2={{$orderby2}}" >描述</a></TD>
 ...
 <td> {{$alldev[t].appgroupname}}</td>
 <td> {{$alldev[t].desc}}</td>
				<td>
				{{if $smarty.session.ADMIN_LEVEL eq 1 or $smarty.session.ADMIN_LEVEL eq 3 or $smarty.session.ADMIN_LEVEL eq 4}}<img src='{{$template_root}}/images/edit_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href='admin.php?controller=admin_app&action=app_group_edit&id={{$alldev[t].id}}'>编辑</a>
				 | <img src='{{$template_root}}/images/bind.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href='admin.php?controller=admin_app&action=appresourcegroup_bind&id={{$alldev[t].id}}'>绑定</a>{{/if}}
				 | <img src='{{$template_root}}/images/delete_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href="#" onClick="if(!confirm('您确定要删除？')) {return false;} else { location.href='admin.php?controller=admin_app&action=app_group_del&gname={{$alldev[t].appgroupname|urlencode}}';}">删除</a>
		    ...
		    <td  colspan="2" align="right">
		   			共{{$total}}个记录  {{$page_list}}  页次：{{$curr_page}}/{{$total_page}}页  {{$items_per_page}}个记录/页  转到第<input name="pagenum" type="text" class="wbk" size="2" onKeyPress="if(event.keyCode==13) window.location='admin.php?controller=admin_app&action=app_group&page='+this.value;">页
		   </td>
                <tr>
