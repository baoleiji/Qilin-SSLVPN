...
function searchit(){
	var url = "admin.php?controller=admin_scp&backupdb_id={{$backupdb_id}}";
...
<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_scp&backupdb_id={{$backupdb_id}}">SCP</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li> 
...
function filter1(){
  ...
  <tr><td><table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
					<tr>
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_scp&orderby1=cliaddr&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">来源地址</a></th>
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_scp&orderby1=svraddr&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">{{$language.DestinationAddress}}</a></th>
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_scp&orderby1=radius_user&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">运维</a></th>
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_scp&orderby1=realname&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">真实姓名</a></th>
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_scp&orderby1=sftp_user&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">{{$language.LocalUser}}</a></th>
						<th class="list_bg"   width="15%"><a href="admin.php?controller=admin_scp&orderby1=start&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">{{$language.StartTime}}</a></th>
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_scp&orderby1=end&orderby2={{$orderby2}}&backupdb_id={{$backupdb_id}}">操作命令</a></th>
					...
					<tr {{if $allsession[t].dangerous > 5}}bgcolor="red"{{elseif $allsession[t].dangerous > 0}}bgcolor="yellow" {{elseif $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}} onmouseover="changeStyle(this,'o');" onmouseout="changeStyle(this,'{{if $allsession[t].dangerous > 5}}red{{elseif $allsession[t].dangerous > 0}}yellow{{elseif $smarty.section.t.index % 2 == 0}}f7f7f7{{/if}}');">
						...
						<td><a href="admin.php?controller=admin_scp&cliaddr={{$allsession[t].cliaddr}}&backupdb_id={{$backupdb_id}}">{{$allsession[t].cliaddr}}</a></td>
					
						<td><a href="admin.php?controller=admin_scp&svraddr={{$allsession[t].svraddr}}&backupdb_id={{$backupdb_id}}">{{$allsession[t].svraddr}}</a></td>
						<td><a href="admin.php?controller=admin_scp&radius_user={{$allsession[t].radius_user}}&backupdb_id={{$backupdb_id}}">{{$allsession[t].radius_user}}</a></td>
						<td><a href="admin.php?controller=admin_scp&realname={{$allsession[t].realname|urlencode}}&backupdb_id={{$backupdb_id}}">{{$allsession[t].realname}}</a></td>
						<td><a href="admin.php?controller=admin_scp&sftp_user={{$allsession[t].sftp_user}}&backupdb_id={{$backupdb_id}}">{{$allsession[t].sftp_user}}</a></td>
						
						<td>{{$allsession[t].at}}</ td>
						<td>{{$allsession[t].comm}}</td>
						<td style="TEXT-ALIGN: left;">{{if $allsession[t].filename}}<img src="{{$template_root}}/images/down.gif" width="16" height="16" align="absmiddle"> <a href="admin.php?controller=admin_scp&action=download&cid={{$allsession[t]['cid']}}"  target="hide">下载</a>{{/if}}
						...
						{{if 0}} {{if !$backupdb_id}}| <img src="{{$template_root}}/images/scico.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_scp&action=del_session&sid={{$allsession[t]['sid']}}&backupdb_id={{$backupdb_id}}">{{$language.Delete}}</a>{{/if}}{{/if}} <!-- | <a href=admin.php?controller=admin_scp&action=download&sid={{$allsession[t]['sid']}} target="_blank">{{$language.Log}}</a>--></td>
					</tr>
					{{/section}}
					<tr>
