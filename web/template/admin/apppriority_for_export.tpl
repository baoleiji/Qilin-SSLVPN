  ...
  <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0" >
        <tr >
          ...
          <td height="35" bgcolor="#528AC7" class="main_title">{{$language.ReportManager}}——应用权限报表</td>         
  ...
  <tr>
	<td class="main_content">
		<table bordercolor="white" cellspacing="1" cellpadding="5" border="0" width="100%"  class="BBtable">
					...
					{{if $type eq 'luser'}}<th class="list_bg" >运维用户</TD><th class="list_bg" >别名</TD>
					{{elseif $type eq 'lgroup'}}<th class="list_bg" >{{$language.UserGroup}}</TD>{{/if}}
                    <th class="list_bg" >设备IP</TD>
                    <th class="list_bg" >应用发布IP</TD>
                    <th class="list_bg" >应用名称</TD>
					<th class="list_bg" >应用用户名</TD>
					<th class="list_bg" >自动修改密码</TD>
					<th class="list_bg" >激活</TD>
					</tr>
					{{section name=t loop=$alldev}}
					<tr {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						<td>{{$smarty.section.t.index+1}}</td>
						{{if $type eq 'luser'}}<td>{{$alldev[t].webuser}}</td><td>{{$alldev[t].webrealname}}</td>
