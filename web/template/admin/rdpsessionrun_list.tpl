...
<ul>
    ...
    <li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_rdprun">RDP实时监控</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li>
   ...
   <tr>
    <td class="main_content"><form action="admin.php?controller=admin_session&backupdb_id={{$backupdb_id}}" method="post" name="search" >
...
cal.manageFields("f_rangeStart_trigger", "f_rangeStart", "%Y-%m-%d %H:%M:%S");
  ...
  <tr><td><table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
				...
				<th class="list_bg"  width="10%">开始时间</th>
					...
					<td><a href="admin.php?controller=admin_rdprun&user={{$allsession[t].user}}">{{$allsession[t].luser}}</a></td>
					<td><a href="admin.php?controller=admin_rdprun&realname={{$allsession[t].realname|urlencode}}">{{$allsession[t].realname}}</a></td>
					<td><a href="admin.php?controller=admin_rdprun&luser={{$allsession[t].luser}}">{{$allsession[t].user}}</a></td>
					<td><a href="admin.php?controller=admin_session&addr={{$allsession[t].cli_addr}}">{{$allsession[t].cli_addr}}</a></td>
					
					<td><a href="admin.php?controller=admin_rdprun&addr={{$allsession[t].addr}}">{{$allsession[t].addr}}</a></td>
					
					<td>{{$allsession[t].start}}</ td>
					<td style="TEXT-ALIGN: left;"><img src="{{$template_root}}/images/036.gif" width="16" height="16" align="absmiddle">
					<a  id="p_{{$allsession[t]['sid']}}"  onclick="return go('{{$curr_url}}&mstsc=1&sid={{$allsession[t]['sid']}}','p_{{$allsession[t]['sid']}}')" href="#" target="hide">监控</a>&nbsp;
					{{*| <img src="{{$template_root}}/images/ie.png" width="16" height="16" align="absmiddle"><a href='{{$curr_url}}&activex=1&sid={{$allsession[t]['sid']}}' target="_blank">ACTIVEX</a>&nbsp;*}}| <img src="{{$template_root}}/images/disconnect.png" width="16" height="16" align="absmiddle">
					<a href="admin.php?controller=admin_rdprun&action=cutoff&sid={{$allsession[t]['sid']}}" >断开</a>

						
						{{if $admin_level == 2}} | <img src="{{$template_root}}/images/scico.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_rdprun&action=del_session&sid={{$allsession[t]['sid']}}">删除</a>{{/if}}</td>
					</tr>
					{{/section}}
					<tr>
