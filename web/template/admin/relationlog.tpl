...
<div class="menu">
<ul>
<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_relationlog">关联日志</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li> 
<li class="me_b"><img src="{{$template_root}}/images/an11.jpg" align="absmiddle"/><a href="admin.php?controller=admin_relationlog&action=relationidslog">报警日志</a><img src="{{$template_root}}/images/an33.jpg" align="absmiddle"/></li>
<li class="me_b"><img src="{{$template_root}}/images/an11.jpg" align="absmiddle"/><a href="admin.php?controller=admin_relationlog&action=relationserverlog">主机日志</a><img src="{{$template_root}}/images/an33.jpg" align="absmiddle"/></li>
  ...
  <tr>
    ...
    <FORM method=get name=f1   onSubmit="return false;"    action=admin.php>
         <INPUT id="controller " value=admin_relationlog type=hidden name=c>
      <INPUT id=action value=index type=hidden   name=a> 	
		      报警设备:<INPUT name=hostname  size="12" class="wbk"> 
		      <INPUT  size="12" class="an_02"  onClick="JavaScript: document.getElementById('action').value='index';submit();" value=查找  type=button> 
	</FORM>
</td>
  </tr>
</table>	
  ...
  <table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
	 <FORM method=post name=list     action=admin.php?controller=admin_relation&amp;a=delete_all&amp;t=relationlog>
					
				<tr>
			 	     <TH width="2%"  class="list_bg">选</TH>
			          <TH width="3%"    class="list_bg">ID</TH>
			          <TH  width="8%"  class="list_bg">报警设备</TH>
			          <TH width="8%"   class="list_bg">服务器</TH>
			          <TH  width="3%"   class="list_bg">级别</TH>
					
					 <TH  width="6%"  class="list_bg">状态</TH>
					  <TH width="35%"    class="list_bg">事件</TH>
					 <TH width="8%"  class="list_bg"  >操作</TH>
					</tr>
  			 ...
  			 <TR class="list_tr_bg{{if $smarty.section.t.index % 2 ne 0}}1{{/if}}">
			          ...
			          <TD>{{$all[t].idsip}}</TD>
			          <TD>{{$all[t].serverip}}</TD>
			          <TD>{{$all[t].level}}</TD>
			         
			          <TD>
				          {{if $all[t].status == 1}}已处理
						  {{else }}未处理
						  {{/if}}
			          </TD>
			            <TD>{{$all[t].idsmsg}}</TD>
			          <TD >
		  				<input name="detail" type="button" value="详细"  onclick="javascript:window.open('admin.php?controller=admin_relationlog&action=detail&t=relationlog&id={{$all[t].seq}}','newwin')"  class="bnnew2"  />
			          </TD>
					</tr>
					{{/section}}
		<TR>
