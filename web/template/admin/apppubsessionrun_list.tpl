...
<script>
function searchit(){
	url = "admin.php?controller=admin_apppubrun";
...
<ul>
	...
	<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_apppubrun">应用发布实时监控</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li>
</ul>
</div></td></tr>
...
cal.manageFields("f_rangeStart_trigger", "f_rangeStart", "%Y-%m-%d %H:%M:%S");
  ...
  <tr><td><table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
					<tr>
						...
						<th class="list_bg"   width="13%">详细信息</th>
					</tr>
					...
					<tr {{if $allsession[t].dangerous > 1}}bgcolor="red"{{elseif $allsession[t].dangerous > 0}}bgcolor="yellow" {{elseif $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}  onmouseover="changeStyle(this,'o');" onmouseout="changeStyle(this,'{{if $allsession[t].dangerous > 1}}red{{elseif $allsession[t].dangerous > 0}}yellow{{elseif $smarty.section.t.index % 2 == 0}}f7f7f7{{/if}}');">
										...
										<td><a href="admin.php?controller=admin_apppubrun&cli_addr={{$allsession[t].cli_addr}}">{{$allsession[t].cli_addr}}</a></td>
					
					<td><a href="admin.php?controller=admin_apppubrun&addr={{$allsession[t].addr}}">{{$allsession[t].addr}}</a></td>
					
					<td><a href="admin.php?controller=admin_apppubrun&luser={{$allsession[t].luser}}">{{$allsession[t].luser}}</a></td>
					
					<td><a href="admin.php?controller=admin_apppubrun&realname={{$allsession[t].realname|urlencode}}">{{$allsession[t].realname}}</a></td>
					<td><a href="admin.php?controller=admin_apppubrun&user={{$allsession[t].user}}">{{$allsession[t].user}}</a></td>
					...
					<a  id="p_{{$allsession[t]['sid']}}"  onclick="return go('{{$curr_url}}&mstsc=1&sid={{$allsession[t]['sid']}}','p_{{$allsession[t]['sid']}}')" href="#" target="hide">监控</a>&nbsp;{{*<img src="{{$template_root}}/images/ie.png" width="16" height="16" align="absmiddle">
					<a href='{{$curr_url}}&activex=1&sid={{$allsession[t]['sid']}}' target="_blank">ACTIVEX</a>&nbsp;*}}| <img src="{{$template_root}}/images/disconnect.png" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_vncrun&action=cutoff&sid={{$allsession[t]['sid']}}" >断开</a>
