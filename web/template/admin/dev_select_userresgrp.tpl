...
<ul>
  ...
  <tr>
	...
	<form name="f1" method=post OnSubmit='return check()' action="admin.php?controller=admin_member&action=protect_user_resgrp_save&uid={{$uid}}">
	<tr bgcolor="f7f7f7">
	...
	<tr>
		...
		<td width="150"><input type="checkbox" name='Check{{$smarty.section.g.index}}' value='{{$alldevgroup[g].id}}'  {{$alldevgroup[g].check}}><a onclick="window.open ('admin.php?controller=admin_pro&action=resourcegrp_seluser&uid={{$uid}}&sid={{$alldevgroup[g].id}}&sessionluser={{$sessionluser}}', 'newwindow', 'height=330, width=400, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=yes,location=no, status=no');return false;" href="#" target="_blank" >{{$alldevgroup[g].groupname}}</a></td>{{if ($smarty.section.g.index +1) % 5 == 0}}</tr><tr>{{/if}}
		{{/section}}
		</tr></table>
	  </td>
	</tr>
