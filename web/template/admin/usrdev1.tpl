...
{{/section}}
				...
				<td class="td_line" width="30%">
					...
					<a id="a{{$alldev[t].id}}1" onclick="rdpgo({{$alldev[t].id}});return false;" href='admin.php?controller=admin_pro&rdptype=activex&action=dev_login&id={{$alldev[t].id}}'>本地</a>
...
function rdpgo(iid){
	...
	var href = document.getElementById('a'+iid+'1').href;
	href=changeQueryString(href,'selectedip',lbip);
	href=changeQueryString(href,'screen',fenbian);
	document.getElementById('a'+iid+'2').href =href;
	//alert(hid.src);
...
function rdpgo2(iid){
	...
	var hid = document.getElementById('hide');
	var lbip = document.getElementById('lb'+iid).options[document.getElementById('lb'+iid).options.selectedIndex].value;
	var href = document.getElementById('a'+iid+'2').href;
	href=changeQueryString(href,'selectedip',lbip);
	href=changeQueryString(href,'screen',fenbian);
	document.getElementById('a'+iid+'2').href=href;
	//alert(hid.src);
...
function goto3(iid){
	...
	document.getElementById(iid).href=changeQueryString(href,'selectedip',lbip);
...
function changeQueryString(url, name, value){
	...
	if(pos>0){
		var end = url.indexOf('&',pos+2+name.length);
		url = url.substring(0,pos+2+name.length) + value + url.substring(end);
	}else{
		url = url+'&'+name+'='+value;
	}
	return url;
