  ...
  <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0" >
        <tr >
          ...
          <td height="35" bgcolor="#528AC7" class="main_title">{{$language.ReportManager}}——系统权限报表</td>         
		...
		<table bordercolor="white" cellspacing="1" cellpadding="5" border="0" width="100%"  class="BBtable">
					...
					{{if $type eq 'luser'}}
					 ...
					 {{elseif $type eq 'lgroup'}}
					  <th class="list_bg" >{{$language.UserGroup}}</TD>
					 {{/if}}
					{{if $type eq 'luser'}}<th class="list_bg" >运维组</TD>{{/if}}
                    <th class="list_bg" >{{$language.device}}IP</TD>
                    <th class="list_bg" >{{$language.System}}{{$language.User}}</TD>
                    <th class="list_bg" >{{$language.Black}}</TD>
					<th class="list_bg" >来源IPv4</TD>
					<th class="list_bg" >来源IPv6</TD>
					<th class="list_bg" >自动为超级用户</TD>
					<th class="list_bg" >SYSLOG告警</TD>					
					<th class="list_bg" >Mail告警</TD>
					<th class="list_bg" >账号锁定</TD>
					{{if $type eq 'user'}}<th class="list_bg" >{{$language.Logintime}}</TD>{{/if}}
					...
					<tr {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						...
						<td>{{$alldev[t].device_ip}}({{$alldev[t].hostname}})</td>
