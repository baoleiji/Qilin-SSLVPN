...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
   <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0" >
        <tr >
          ...
          <td height="35" bgcolor="#528AC7" class="main_title">{{$language.ReportManager}}——{{$title}}</td>         
  ...
  <tr>
	<td class="main_content">
		<table bordercolor="white" cellspacing="1" cellpadding="5" border="0" width="100%"  class="BBtable"><tr>
					<td colspan="9">日期:{{$f_rangeStart}} 到 {{$f_rangeEnd}}</td>
					</tr>
					<tr>
						<th class="list_bg"  bgcolor="d9ecfa" width="8%">{{$language.SourceAddress}}</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="8%">审计系统</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="8%">{{$language.Ipaddress}}</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="6%">登录协议</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="10%">时间</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="10%">运维用户</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="10%">别名</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="8%">运维组</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="10%">{{$language.Username}}</th>						
						<th class="list_bg"  bgcolor="d9ecfa" width="10%">{{if $alllog[t].authenticationstatus}}成功{{else}}失败{{/if}}</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="">出错原因</th>
					</tr>
					{{section name=t loop=$alllog}}
					<tr {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						<td>{{$alllog[t].sourceip}}</td>
						<td>{{$alllog[t].auditip}}</td>
						<td>{{$alllog[t].serverip}}</td>
						<td>{{$alllog[t].portocol}}</td>
						<td>{{$alllog[t].time}}</td>
						<td>{{$alllog[t].audituser}}</td>
						<td>{{$alllog[t].realname}}</td>
						<td>{{$alllog[t].groupname}}</td>
						<td>{{$alllog[t].systemuser}}</td>
						<td>{{$alllog[t].status}}</td>
