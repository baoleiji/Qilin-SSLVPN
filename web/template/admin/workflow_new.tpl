  ...
  <tr>
	<td class="">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="BBtable">
          <tr>
            <td align="center"><form name="f1" method=post OnSubmit='return check()' action="admin.php?controller=admin_workflow&action=workflow_new_save&sid={{$sip['sid']}}">
	...
	<tr bgcolor="f7f7f7">
		...
		<td width="67%">
		<input type="text" name="device_ip" value="{{$dev.device_ip}}" />
	  </td>
	</tr>	
	<tr bgcolor="">
		<td width="33%" align=right>
		...
		<td width="67%">
		<input type="text" name="username" value="{{$dev.username}}" />
	  </td>
	</tr>
	<tr bgcolor="f7f7f7">
		<td width="33%" align=right>
		...
		<option value="{{$methods[a].id}}" {{if $dev['login_template'] eq $methods[a].id}}selected{{/if}} >{{$methods[a].login_method}}</option>
		{{/section}}
		</select>
	  </td>
	</tr>
	<tr bgcolor="">
		...
		<td width="67%">
		<input type="text" name="port" value="{{$dev.port}}" />
	  </td>
	</tr>
	<tr bgcolor="">
		<td width="33%" align=right>
		...
		<td width="67%">
		<INPUT value="{{if $member.end_time ne '2037-01-01 00:00:00'}}{{$member.end_time}}{{/if}}" id="deadline" name="deadline" onFocus="setday(this)">&nbsp;&nbsp;<input type="button"  id="f_rangeEnd_trigger" name="f_rangeEnd_trigger" value="选择时间" class="wbk"> 
  <script type="text/javascript">
