      ...
      <tr  >
        <td class="">
		  ...
		  <TABLE class=BBtable cellSpacing=0 cellPadding=0 width="100%"             align=center>
              <TBODY>
              <TR>
                <TD width="6%" class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_server_policy&orderby1=name&orderby2={{$orderby2}}">名称</a></TD>
				<TD width="8%" class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_server_policy&orderby1=cpuhigh&orderby2={{$orderby2}}">CPU</a></TD>
				<TD width="8%" class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_server_policy&orderby1=cpuiohigh&orderby2={{$orderby2}}">CPU-IO</a></TD>
                <TD width="8%"  class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_server_policy&orderby1=memoryhigh&orderby2={{$orderby2}}">内存</a></TD>
                <TD width="8%"  class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_server_policy&orderby1=swaphigh&orderby2={{$orderby2}}">交换空间</a></TD>
                <TD width="8%"  class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_server_policy&orderby1=diskhigh&orderby2={{$orderby2}}">存储</a></TD>
				<TD width="10%"  class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_server_policy&orderby1=cpurrd&orderby2={{$orderby2}}">cpu是否记录</a></TD>
				<TD width="10%"  class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_server_policy&orderby1=memoryrrd&orderby2={{$orderby2}}">内存是否记录</a></TD>
				<TD width="10%"  class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_server_policy&orderby1=swaprrd&orderby2={{$orderby2}}">交换空间是否记录</a></TD>
				<TD width="10%"  class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_server_policy&orderby1=diskrrd&orderby2={{$orderby2}}">存储是否记录</a></TD>
			...
			<TD class="" style="text-align:left">{{$tholdlist[g].cpulow}}%-{{$tholdlist[g].cpuhigh}}%</TD>
			<TD class="" style="text-align:left">{{$tholdlist[g].cpuiolow}}%-{{$tholdlist[g].cpuiohigh}}%</TD>
			<TD class="" style="text-align:left">{{$tholdlist[g].memorylow}}%-{{$tholdlist[g].memoryhigh}}%</TD>
			<TD class="" style="text-align:left">{{$tholdlist[g].swaplow}}%-{{$tholdlist[g].swaphigh}}%</TD>
			<TD class="" style="text-align:left">{{$tholdlist[g].disklow}}%-{{$tholdlist[g].diskhigh}}%</TD>
			<TD class="" style="text-align:left">{{if $tholdlist[g].cpurrd}}是{{else}}否{{/if}}</TD>
			<TD class="" style="text-align:left">{{if $tholdlist[g].memoryrrd}}是{{else}}否{{/if}}</TD>
			<TD class="" style="text-align:left">{{if $tholdlist[g].swaprrd}}是{{else}}否{{/if}}</TD>
			<TD class="" style="text-align:left">{{if $tholdlist[g].diskrrd}}是{{else}}否{{/if}}</TD>
			<td>
			<img src='{{$template_root}}/images/edit_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href='admin.php?controller=admin_thold&action=snmp_server_policy_edit&id={{$tholdlist[g].id}}'>修改</a> 
			| <img src='{{$template_root}}/images/edit_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href='admin.php?controller=admin_thold&action=snmp_server_policy_set&id={{$tholdlist[g].id}}'>设置</a> 
			| <img src='{{$template_root}}/images/delete_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href="#" onClick="if(!confirm('您确定要删除？')) {return false;} else { location.href='admin.php?controller=admin_thold&action=snmp_server_policy_del&id={{$tholdlist[g].id}}';}">删除</a></td>
			</TR>
			{{/section}}
			<tr><td colspan="10" align="right"><input type="button" onclick="location.href='admin.php?controller=admin_thold&action=snmp_server_policy_edit'" name="edit" value="添加" class="an_02">&nbsp&nbsp;&nbsp;共{{$total}}个记录  {{$page_list}}  页次：{{$curr_page}}/{{$total_page}}页  {{$items_per_page}}个记录/页  转到第<input name="pagenum" type="text" class="wbk" size="2" onKeyPress="if(event.keyCode==13) window.location='admin.php?controller=admin_thold&action=status_thold&page='+this.value;">页&nbsp;&nbsp;&nbsp;{{if $smarty.session.ADMIN_LEVEL eq 3}}<a href="{{$curr_url}}&derive=1" target="hide">{{$language.ExportresulttoExcel}}</a>{{/if}}</td></tr>
			</form>
			</TBODY>
		</TABLE>
