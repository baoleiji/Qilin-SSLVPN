...
<form name="f1" method=post OnSubmit='return check()' action="admin.php?controller=admin_config&action=ldapimportconfig_save&id={{$adconfig.id}}">
<tr><th colspan="6" class="list_bg">选择已有配置:<select onchange="window.location='admin.php?controller=admin_config&action=ldapimportconfig&id='+this.value">
...
{{/section}}
	...
	<tr {{if $trnumber % 2 == 0}}bgcolor="f7f7f7"{{/if}}><td align="right">LDAP用户DN:</td>		
	<td>
		<input type="text" class="wbk" size="60" name="adusername" value="{{$adconfig.adusername}}" />	
		</td>	
	</tr>
	<tr {{if $trnumber % 2 == 0}}bgcolor="f7f7f7"{{/if}}><td align="right">LDAP密码:</td>		
	<td>
		<input type="password" class="wbk" name="adpassword" value="{{$adconfig.adpassword}}" />
		</td>	
	</tr>
	...
	<tr>
		<td  align="center" colspan=2><input type="submit" name="submit"  value="同步" class="an_02">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit"  value="{{$language.Save}}" name="submit" class="an_02">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" onclick="window.location='admin.php?controller=admin_config&action=ldapimportconfig&id=-1'"  value="新建" class="an_02"></td>
		</tr>
