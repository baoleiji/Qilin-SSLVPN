...
<li class="me_b"><img src="{{$template_root}}/images/an11.jpg" align="absmiddle"/><a href="admin.php?controller=admin_alert">告警配置</a><img src="{{$template_root}}/images/an33.jpg" align="absmiddle"/></li> 
  ...
  <tr>
    ...
    <FORM method=get name=f1   onSubmit="return false;"    action=admin.php>
      ...
      <INPUT id=action value=index type=hidden   name=a> 
		    日志规则:<INPUT name=hostname  size="12" class="wbk"> 
		      <INPUT  size="12" class="bnnew2"  onClick="JavaScript: document.getElementById('action').value='eventconfig';submit();" value=查找  type=button> 
	</FORM>
</td>
  </tr>
</table>	
  ...
  <table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
	 <FORM method=post name=list     action=admin.php?controller=admin_eventlogs&amp;a=delete_all&amp;t=eventconfig>
					
				<tr>
			 	     <TH width="3%" class="list_bg">选</TH>
			          <TH width="3%" class="list_bg">ID</TH>
			          <TH width="10%" class="list_bg">日志规则</TH>
			           <TH width="8%" class="list_bg">事件名称</TH>
			          <TH width="3%" class="list_bg">级别</TH>
	
			          <TH width="15%" class="list_bg">描述</TH>
  			 ...
  			 <TR class="list_tr_bg{{if $smarty.section.t.index % 2 ne 0}}1{{/if}}">
			          ...
			          <TD>{{$all[t].desc}}</TD>
                 <TD >
                 <input name="edit" type="button" value="编辑"  onclick="javascript:location.href='admin.php?controller=admin_eventlogs&action=edit&t=eventconfig&id={{$all[t].seq}}'"  class="bnnew2" />
                 </TD>
		...
		<TR>
          ...
          <INPUT  onclick="javascript:for(var i=0;i<this.form.elements.length;i++){var e=this.form.elements[i];if(e.name=='chk_member[]')e.checked=this.form.select_all.checked;}" 
            value=checkbox type=checkbox name=select_all>选本页显示的所有记录&nbsp;&nbsp;<INPUT class=an_06 onClick="my_confirm('删除所选记录');if(!chk_form())  return false;" value=批量删除所选记录 type=submit>      
             ...
             <input type="button" onclick="window.location='admin.php?controller=admin_eventlogs&action=edit&t=eventconfig'"  value=" 添加 "  class="an_02">
          <input name="export" type="button" value="导入记录"  onclick="javascript:location.href='admin.php?controller=admin_eventlogs&action=import'" class="an_02" />
         <input name="export" type="button" value="导出记录"  onclick="javascript:location.href='admin.php?controller=admin_eventlogs&action=export'" class="an_02" />
