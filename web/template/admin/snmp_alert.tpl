...
function searchit(){
	document.f1.action = "admin.php?controller=admin_thold&action=snmp_alert";
	document.f1.action += "&name="+document.f1.name.value;
	...
	<td align="left" colspan = "7"><table width="100%" border="0" cellspacing="0" cellpadding="0"  class="main_content">
                ...
                <TBODY>
				 <TR>
					...
					名称<input type="text" class="wbk" name="name">
      ...
      <tr  >
        <td class="">
		  ...
		  <TABLE class=BBtable cellSpacing=0 cellPadding=0 width="100%"             align=center>
              <TBODY>
              <TR>
                <TD width="12%" class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_alert&orderby1=name&orderby2={{$orderby2}}">名称</a></TD>
				<TD width="12%" class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_alert&orderby1=groupname&orderby2={{$orderby2}}">设备组</a></TD>
                <TD width="12%"  class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_alert&orderby1=mail_alarm&orderby2={{$orderby2}}">邮件告警</a></TD>
                <TD width="12%"  class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_alert&orderby1=sms_alarm&orderby2={{$orderby2}}">短信告警</a></TD>
                <TD width="12%"  class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_alert&orderby1=enable&orderby2={{$orderby2}}">允许</a></TD>
                <TD width="12%"  class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_alert&orderby1=period&orderby2={{$orderby2}}">告警周期</a></TD>
                <TD width="12%"  class="list_bg"><a href = "admin.php?controller=admin_thold&action=snmp_alert&orderby1=alertitem&orderby2={{$orderby2}}">告警项</a></TD>
			...
			<TD class="" style="text-align:left">{{if $tholdlist[g].groupname}}{{$tholdlist[g].groupname}}{{else}}全部{{/if}}</TD>
			<TD class="" style="text-align:left">{{if $tholdlist[g].mail_alarm eq 1}}是{{else}}否{{/if}}</TD>
			<TD class="" style="text-align:left">{{if $tholdlist[g].sms_alarm eq 1}}是{{else}}否{{/if}}</TD>
			<TD class="" style="text-align:left">{{if $tholdlist[g].enable}}是{{else}}否{{/if}}</TD>
			<TD class="" style="text-align:left">{{$tholdlist[g].period}}</TD>
			<TD class="" style="text-align:left">{{if $tholdlist[g].alarmitem eq 'system'}}系统{{elseif $tholdlist[g].alarmitem eq 'network'}}网络{{else}}应用{{/if}}</TD>
			<td>
			
			<img src='{{$template_root}}/images/edit_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href='admin.php?controller=admin_thold&action=snmp_alert_edit&id={{$tholdlist[g].seq}}'>修改</a> 
			| <img src='{{$template_root}}/images/delete_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href="#" onClick="if(!confirm('您确定要删除？')) {return false;} else { location.href='admin.php?controller=admin_thold&action=snmp_alert_del&id={{$tholdlist[g].seq}}';}">删除</a></td>
			</TR>
			{{/section}}
			<tr><td colspan="7" align="right"><input type="button" onclick="location.href='admin.php?controller=admin_thold&action=snmp_alert_edit'" name="edit" value="添加" class="an_02">&nbsp&nbsp;&nbsp;共{{$total}}个记录  {{$page_list}}  页次：{{$curr_page}}/{{$total_page}}页  {{$items_per_page}}个记录/页  转到第<input name="pagenum" type="text" class="wbk" size="2" onKeyPress="if(event.keyCode==13) window.location='admin.php?controller=admin_thold&action=status_thold&page='+this.value;">页&nbsp;&nbsp;&nbsp;{{if $smarty.session.ADMIN_LEVEL eq 3}}<a href="{{$curr_url}}&derive=1" target="hide">{{$language.ExportresulttoExcel}}</a>{{/if}}</td></tr>
			</form>
			</TBODY>
		</TABLE>
