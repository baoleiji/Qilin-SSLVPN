  ...
  <tr>
	<td colspan="2" class="">
		<table bordercolor="white" cellspacing="1" cellpadding="5" border="0" width="100%"  class="BBtable">
			<tr>
				<th class="list_bg"  width="10%">执行时间</th>
				<th class="list_bg"  width="10%">运维用户</th>
				<th class="list_bg"  width="10%">数据库服务器</th>
				<th class="list_bg"  width="10%">数据库类型</th>
				<th class="list_bg"  width="60%">SQL</th>
			</tr>
			{{section name=t loop=$allcommand}}
			<tr {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
				<td>{{$allcommand[t].username}}</ td>
				<td>{{$allcommand[t].excutetime}}</ td>
				<td>{{$allcommand[t].dbip}}</td>
				<td>{{$allcommand[t].dbtype}}</td>
				<td>{{$allcommand[t].sqltext}}</td>
			</tr>
			{{/section}}
			<tr>
				<td colspan="12" align="right">
					{{$language.all}}{{$command_num}}{{$language.Command}}  {{$page_list}}  {{$language.Page}}：{{$curr_page}}/{{$total_page}}{{$language.page}}  {{$items_per_page}}{{$language.item}}{{$language.Log}}/{{$language.page}}  {{$language.Goto}}<input name="pagenum" type="text" class="wbk" size="2" onKeyPress="if(event.keyCode==13) window.location='admin.php?controller=admin_apppub&action=sqlview&backupdb_id={{$backupdb_id}}&page='+this.value;">{{$language.page}}  
				<!--
