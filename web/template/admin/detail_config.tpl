  ...
  <tr><td><table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
		<form name="f1" method=post action="admin.php?controller=admin_autorun&action=autobackup_delete&type={{$smarty.get.type}}">
			<tr>
				...
				<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_autorun&action=detail_config&orderby1=name&orderby2={{$orderby2}}" >脚步名称</a></th>
				<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_autorun&action=detail_config&orderby1=scriptpath&orderby2={{$orderby2}}" >脚本所在行号</a></th>
				<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_autorun&action=detail_config&orderby1=desc&orderby2={{$orderby2}}" >动作</a></th>
				<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_autorun&action=detail_config&orderby1=desc&orderby2={{$orderby2}}" >正则</a></th>
				<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_autorun&action=detail_config&orderby1=desc&orderby2={{$orderby2}}" >范围</a></th>
			...
			<td><input type="checkbox" name="chk_member[]" value="{{$alldev[t].id}}"></td>
				<td>{{$alldev[t].name}}</td>
				<td>{{$alldev[t].line_number}}</td>
				<td>{{$alldev[t].action}}</td>
				<td>{{$alldev[t].regex}}</td>
				<td>{{$alldev[t].low_value}}-{{$alldev[t].high_value}}</td>
				<td style="TEXT-ALIGN: left;"><img src='./template/admin/images/edit_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href='admin.php?controller=admin_autorun&action=detail_config_edit&id={{$alldev[t].id}}'>修改</a>| <img src='./template/admin/images/delete_ico.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href='admin.php?controller=admin_autorun&action=detail_config_del&id={{$alldev[t].id}}'>删除</a></td>
			</tr>
			{{/section}}
			
			<tr>
				<td  colspan="3" align="left">	<input name="select_all" type="checkbox" onclick="javascript:for(var i=0;i<this.form.elements.length;i++){var e=this.form.elements[i];if(e.name=='chk_member[]')e.checked=this.form.select_all.checked;}" value="checkbox">&nbsp;&nbsp;<input type="submit"  value="删除" onclick="my_confirm('{{$language.DeleteUsers}}');if(chk_form()) document.f1.action='admin.php?controller=admin_autorun&action=detail_config_del'; else return false;" class="an_02">
				&nbsp;&nbsp;
				<input type="button" onclick="window.location='admin.php?controller=admin_autorun&action=detail_config_edit'"  value="增加"  class="an_02">
