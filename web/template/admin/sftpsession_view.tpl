...
</ul><span class="back_img"><A href="admin.php?controller=admin_sftp&back=1"><IMG src="{{$template_root}}/images/back1.png" 
  ...
  <tr>
	<td class=""><table bordercolor="white" cellspacing="1" cellpadding="5" border="0" width="100%" class="BBtable">
			<tr>
				...
				<th class="list_bg"  width="50%">{{$language.Command}}</th>
			...
			<tr {{if $allcommand[t].dangerlevel > 5}}bgcolor="red"{{elseif $allcommand[t].dangerlevel > 0}}bgcolor="yellow" {{elseif $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}} onmouseover="changeStyle(this,'o');" onmouseout="changeStyle(this,'{{if $allcommand[t].dangerlevel > 5}}red{{elseif $allcommand[t].dangerlevel > 0}}yellow {{elseif $smarty.section.t.index % 2 == 0}}f7f7f7{{/if}}');">
				...
				<td>{{if $allcommand[t].successed eq 1}}{{$language.Secuess}}{{else}}{{$language.Failed}}{{/if}}</td>
				<td>{{$allcommand[t].backupsize/1024|string_format:"%.2f"}}</td>
				<td>{{if $allcommand[t].backupflag}}<a href="admin.php?controller=admin_sftp&action=download&cid={{$allcommand[t].cid}}&backupdb_id={{$backupdb_id}}" target="{{if $backupdb_id}}_blank{{else}}hide{{/if}}">{{$language.Download}}</a>{{/if}}</td>
			</tr>
			{{/section}}
			<tr>
				<td colspan="12" align="right">
					{{$language.all}}{{$command_num}}{{$language.Command}}  {{$page_list}}  {{$language.Page}}：{{$curr_page}}/{{$total_page}}{{$language.page}}  {{$items_per_page}}{{$language.item}}{{$language.Log}}/{{$language.page}}  {{$language.Goto}}<input name="pagenum" type="text" size="2" onKeyPress="if(event.keyCode==13) window.location='admin.php?controller=admin_sftp&action=view&sid={{$sid}}&backupdb_id={{$backupdb_id}}&page='+this.value;">{{$language.page}}
				<!--
