...
<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_log&action=applog_warning">应用告警</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li> 
</ul>
</div>
</td></tr>
	 ...
	 <FORM method=post name=list     action=admin.php?controller=admin_host&amp;a=delete_all&amp;t=host>					
			<tr>		
                ...
                <TH width="10%" class=list_bg>时间</TH>
                  ...
                  <TH  width="10%"  class=list_bg>邮件发送</TH>
                  <TH  width="10%"  class=list_bg>短信发送</TH>
			</tr>
  			 ...
  			 <TR class="list_tr_bg{{if $smarty.section.t.index % 2 ne 0}}1{{/if}}">
                ...
                <TD class=main_list_td1>{{$result_array[r].host}}</TD>
				 <TD class=main_list_td1>{{$result_array[r].datetime}}</TD>
				  ...
				  <TD class=main_list_td1>{{if $result_array[r].mail_status}}成功{{else}}失败{{/if}}</TD>
				  <TD class=main_list_td1>{{if $result_array[r].sms_status}}成功{{else}}失败{{/if}}</TD>
			</tr>
					{{/section}}

				</FORM>	
					<tr><td>
				  	 <input name="export" type="button" value="导出记录"  onclick="javascript:location.href='admin.php?controller=admin_log&action=applog_warning&derive=1'" class="an_02" />      </td>
