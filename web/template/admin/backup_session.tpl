...
<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_status&action=backup_session">双机同步</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li>
...
cal.manageFields("f_rangeEnd_trigger", "f_rangeEnd", "%Y-%m-%d");
  ...
  <tr><td><table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
					<tr>                                                            
						<th class="list_bg"   width="5%">时间</th>
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_status&action=backup_session&orderby1=context&orderby2={{$orderby2}}" >堡垒机IP</a></th>
						
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_status&action=backup_session&orderby1=datetime&orderby2={{$orderby2}}" >会话ID</a></th>
						
						<th class="list_bg"   width="5%"><a href="admin.php?controller=admin_status&action=backup_session&orderby1=mail_status&orderby2={{$orderby2}}" >状态</a></th>
					
					</tr>
					{{section name=t loop=$reports}}
					<tr {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						<td width="10%">{{$reports[t].datetime}}</td>
						<td width="10%">{{$reports[t].ip_addr}}</td>
						<td width="10%">{{$reports[t].sessionid}}</td>
