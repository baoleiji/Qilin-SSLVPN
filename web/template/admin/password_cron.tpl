...
<form name="f1" method=post OnSubmit='return check()' action="admin.php?controller=admin_pro&action=password_cron">
	...
	<tr bgcolor="f7f7f7"><td>权限列表上传服务调度:</td>
		<td align=left>
	...
	<tr><td>密码文件上传服务:</td>
		<td align=left>
		<input type="checkbox" class="wbk" name="pwduploadservice" value="1" {{if $pwduploadservice}}checked{{/if}} />
		</td>		
	</tr>
	<tr bgcolor="f7f7f7"><td>密码文件上传服务调度:</td>
		<td align=left>
		分钟:<select name="puminute" >
		<option value="*" {{if $puminute eq '*'}}selected{{/if}}>*</option>
		{{section name=pum loop=60}}
		<option value="{{$smarty.section.pum.index}}" {{if $puminute ne '*' and $puminute eq $smarty.section.pum.index}}selected{{/if}}>{{$smarty.section.pum.index}}</option>
		{{/section}}
		</select>&nbsp;&nbsp;&nbsp;&nbsp;
		小时:<select name="puhour" >
		<option value="*" {{if $phour eq '*'}}selected{{/if}}>*</option>
		{{section name=puh loop=24}}
		<option value="{{$smarty.section.puh.index}}" {{if $puhour ne '*' and $puhour eq $smarty.section.puh.index}}selected{{/if}}>{{$smarty.section.puh.index}}</option>
		{{/section}}
		</select>&nbsp;&nbsp;&nbsp;&nbsp;
		天:<select name="puday" >
		<option value="*" {{if puday eq '*'}}selected{{/if}}>*</option>
		{{section name=pud loop=31}}
		<option value="{{$smarty.section.pud.index+1}}" {{if $puday ne '*' and $puday eq $smarty.section.pud.index+1}}selected{{/if}}>{{$smarty.section.pud.index+1}}</option>
		{{/section}}
		</select>&nbsp;&nbsp;&nbsp;&nbsp;
		周:<select name="puweek" >
		<option value="*" {{if $puweek eq '*'}}selected{{/if}}>*</option>
		<option value="0" {{if $puweek eq '0'}}selected{{/if}}>日</option>
		<option value="1" {{if $puweek eq '1'}}selected{{/if}}>一</option>
		<option value="2" {{if $puweek eq '2'}}selected{{/if}}>二</option>
		<option value="3" {{if $puweek eq '3'}}selected{{/if}}>三</option>
		<option value="4" {{if $puweek eq '4'}}selected{{/if}}>四</option>
		<option value="5" {{if $puweek eq '5'}}selected{{/if}}>五</option>
		<option value="6" {{if $puweek eq '6'}}selected{{/if}}>六</option>
		</select>
		</td>		
	</tr>
	<tr>
			<td></td><td align="left"><input type="submit"  value="保存修改" class="an_02">{{if $smarty.session.ADMIN_LEVEL ne 10}}&nbsp;&nbsp;&nbsp;<input type="button" onclick="window.location='admin.php?controller=admin_backup&action=runupload'" value="立即上传运行" class="an_06">&nbsp;&nbsp;&nbsp;<input type="button" onclick="window.location='admin.php?controller=admin_backup&action=runchangepwd'" value="立即改密运行" class="an_06">&nbsp;&nbsp;&nbsp;<input type="button" onclick="window.location='admin.php?controller=admin_backup&action=runtongbu'"  value="立即同步运行" class="an_06">{{/if}}</td>
		</tr>
