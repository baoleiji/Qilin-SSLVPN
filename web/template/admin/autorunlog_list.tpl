...
function searchit(){
	document.search.action = "admin.php?controller=admin_autorunlog";
...
<form action="admin.php?controller=admin_autorunlog" method="post" name="search" >
...
cal.manageFields("f_rangeEnd_trigger", "f_rangeEnd", "%Y-%m-%d %H:%M:%S");
  ...
  <tr><td><table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
					...
					<th class="list_bg" width="5%"><a href="admin.php?controller=admin_auditlog&orderby1=serverip&orderby2={{$orderby2}}">服务器IP</a></th>
						...
						<th class="list_bg" width="4%"><a href="admin.php?controller=admin_auditlog&orderby1=name&orderby2={{$orderby2}}">脚本名称</a></th>	
						<th class="list_bg" width="10%"><a href="admin.php?controller=admin_auditlog&orderby1=checktime&orderby2={{$orderby2}}">脚本执行时间</a></th>
						<th class="list_bg" width="4%"><a href="admin.php?controller=admin_auditlog&orderby1=statuts&orderby2={{$orderby2}}">状态</a></th>					
						<th class="list_bg" width="13%">操作</th>
						
					</tr>
					{{section name=t loop=$allsession}}
					<tr {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						<td><a href="admin.php?controller=admin_auditlog&type=run&addr={{$allsession[t].serverip}}">{{$allsession[t].serverip}}</a></td>			
						<td><a href="admin.php?controller=admin_auditlog&type=run&name={{$allsession[t].servername}}">{{$allsession[t].servername}}</a></td>
						<td><a href="admin.php?controller=admin_auditlog&type=run&message={{$allsession[t].message}}">{{$allsession[t].name}}</a></td>
						<td><a href="admin.php?controller=admin_auditlog&type=run&time={{$allsession[t].backuptime}}">{{$allsession[t].checktime}}</a></td>
						<td><a href="admin.php?controller=admin_auditlog&type=run&status={{$allsession[t].statuts}}">{{if $allsession[t].statuts}}是{{else}}否{{/if}}</a></td>
						<td style="TEXT-ALIGN: left;">
						{{*<img src="{{$template_root}}/images/ico2.gif" width="16" height="16" align="absmiddle"><a href="#" onclick=window.open("admin.php?controller=admin_autorunlog&action=viewfile&sid={{$allsession[t].id}}&start_page=1")>查看</a>*}}
						<img src="{{$template_root}}/images/ico2.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_autorunlog&action=download&type=run&sid={{$allsession[t].id}}&start_page=1" target="hide">下载</a> |
						<img src="{{$template_root}}/images/scico.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_autorunlog&action=delete&type=run&sid={{$allsession[t].id}}">删除</a>
						</td>
					</tr>						
