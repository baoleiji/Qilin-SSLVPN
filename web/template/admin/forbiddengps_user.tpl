...
{{if $smarty.session.ADMIN_LEVEL ne 3 and $smarty.session.ADMIN_LEVEL ne 21 and $smarty.session.ADMIN_LEVEL ne 101}}
  ...
  <tr>
	<td class=""><table bordercolor="white" cellspacing="1" cellpadding="5" border="0" width="100%" class="BBtable">
			<tr>
				<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_forbidden&action=forbiddengps_user&orderby1=device_ip&gid={{$gid}}&orderby2={{$orderby2}}" >地址</a></th>
				<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_forbidden&action=forbiddengps_user&orderby1=hostname&gid={{$gid}}&orderby2={{$orderby2}}" >主机名</a></th>
				<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_forbidden&action=forbiddengps_user&orderby1=username&gid={{$gid}}&orderby2={{$orderby2}}" >系统用户</a></th>
				<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_forbidden&action=forbiddengps_user&orderby1=username&gid={{$gid}}&orderby2={{$orderby2}}" >{{$language.User}}</a></th>
				<th class="list_bg"  width="10%"><a href="admin.php?controller=admin_forbidden&action=forbiddengps_user&orderby1=forbidden_commands_groups&gid={{$gid}}&orderby2={{$orderby2}}" >{{$language.groupname}}</a></th>
			</tr>
			{{section name=t loop=$allcommand}}
			<tr {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
				<td>{{$allcommand[t].device_ip}}</td>
				<td>{{$allcommand[t].hostname}}</td>
				<td>{{$allcommand[t].sysuser}}</td>
				<td>{{$allcommand[t].username}}</td>
