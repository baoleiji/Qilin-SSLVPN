...
<ul>
  ...
  <tr>
	...
	<form name="f1" method=post OnSubmit='return check()' action="admin.php?controller=admin_member&action=protect_groupgrp_save&id={{$id}}&ip={{$ip}}">
	<tr bgcolor="f7f7f7">
	...
	<tr bgcolor="f7f7f7">
		...
		<td width="150"><input type="checkbox" name='Check{{$smarty.section.g.index}}' value='{{$allpass[g].id}}' {{$allpass[g].check}}><a onclick="window.open ('admin.php?controller=admin_pro&action=passedit_selgroup&gid={{$id}}&sid={{$allpass[g].id}}&sessionlgroup={{$sessionlgroup}}', 'newwindow', 'height=330, width=400, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=yes,location=no, status=no');return false;" href="#" target="_blank" >{{$allpass[g].username}}({{$allpass[g].lmname}})</a></td>{{if ($smarty.section.g.index +1) % 5 == 0}}</tr><tr>{{/if}}
		{{/section}}
		</tr></table>
	  </td>
	</tr>
