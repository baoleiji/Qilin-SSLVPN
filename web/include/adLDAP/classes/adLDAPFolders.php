    ...
    */
    public function listing($folderName = NULL, $dnType = adLDAP::ADLDAP_FOLDER, $recursive = NULL, $type = NULL) 
    {
        ...
        if (is_array($folderName)) {
            ...
            echo $searchOu = $ou . ',' . $this->adldap->getBaseDn();
        }
        else {
        ...
        if ($recursive === true) {
            $sr = @ldap_search($this->adldap->getLdapConnection(), $searchOu, $filter, array('objectclass', 'distinguishedname', 'samaccountname','OU','CN'),0,0);
            ...
            $sr = @ldap_list($this->adldap->getLdapConnection(), $searchOu, $filter, array('objectclass', 'distinguishedname', 'samaccountname','OU','CN'));
            $entries = @ldap_get_entries($this->adldap->getLdapConnection(), $sr);
            if (is_array($entries)) {
                return $entries;
            }
