...
class adLDAPUsers {
    ...
    */
    public function info($username, $fields = NULL, $isGUID = false)
    {
        ...
        if ($fields === NULL) { 
            $fields = array("cn","samaccountname","mail","userPrincipalName","memberof","department","displayname","telephonenumber","primarygroupid","objectsid"); 
        }
        if (!in_array("objectsid", $fields)) {
            $fields[] = "objectsid";
        }
        $sr = @ldap_search($this->adldap->getLdapConnection(), $this->adldap->getBaseDn(), $filter, $fields,0,0);
