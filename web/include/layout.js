...
function getForm() {
    return document.getElementsByName("chk2")[0];
}
...
function gt_update_selection_indicators() {
	...
	for (var j = 0; j < frmChk.elements.length; j++) {
		if (frmChk.elements[j].name.substr(0,3) == 'cg_') {
			if (frmChk.elements[j].checked == false) {
				there_are_any_unchecked_ones = true;
			}

			if (!isNaN(frmChk.elements[j].name.substr(3))) {
				lineid = document.getElementById('gt_line' + frmChk.elements[j].name.substr(3));

				if (frmChk.elements[j].checked) {
...
function gt_reset_deps(num_columns) {
	...
	for (var i = 0; i < frmChk.elements.length; i++) {
		if (frmChk.elements[i].name.substr( 0, prefix.length ) == prefix) {
			for (var j = 0; j < num_columns; j++) {
				lineid = document.getElementById('gt_text' + frmChk.elements[i].name.substr(prefix.length) + '_' + j);
				if (lineid) {
					lineid.style.color = '#000000';
				}
			}

			chkbx = document.getElementById('cg_' + frmChk.elements[i].name.substr(prefix.length));
...
function update_selection_indicators() {
	...
	for (var j = 0; j < frmChk.elements.length; j++) {
		if( frmChk.elements[j].name.substr( 0, 4 ) == 'chk_') {
			if (frmChk.elements[j].checked == false) {
				there_are_any_unchecked_ones = true;
			}

			lineid = document.getElementById('line'+ frmChk.elements[j].name.substr(4));
			if (lineid) {
				if (frmChk.elements[j].checked) {
					lineid.style.backgroundColor = 'khaki';
				}else{
...
function dq_update_selection_indicators() {
    ...
    var frmChk = getForm();
	for (var j = 0; j < frmChk.elements.length; j++) {
		if( frmChk.elements[j].name.substr( 0, 3 ) == 'sg_') {
			if (frmChk.elements[j].checked == false) {
				there_are_any_unchecked_ones = true;
			}

			lineid = document.getElementById('line'+ frmChk.elements[j].name.substr(3));
			if (lineid) {
				if (frmChk.elements[j].checked) {
					lineid.style.backgroundColor = 'khaki';
				}else{
...
function dq_update_deps(snmp_query_id, num_columns) {
    ...
    $('#cur_snmp_status').html(created_graphs[snmp_query_graph_id].length);
    
	for (var i = 0; i < created_graphs[snmp_query_graph_id].length; i++) {
		for (var j = 0; j < num_columns; j++) {
			lineid = document.getElementById('text' + snmp_query_id + '_' + created_graphs[snmp_query_graph_id][i] + '_' + j);
			if ( lineid ) { lineid.style.color = '#33D041' };
		}
...
function dq_reset_deps(snmp_query_id, num_columns) {
    ...
    //document.getElementById('cur_snmp_status').innerHTML = "0";
    
	for (var i = 0; i < frmChk.elements.length; i++) {
		if (frmChk.elements[i].name.substr( 0, prefix.length ) == prefix) {
			for (var j = 0; j < num_columns; j++) {
				lineid = document.getElementById('text' + snmp_query_id + '_' + frmChk.elements[i].name.substr(prefix.length) + '_' + j);
				lineid.style.color = '#000000';
			}

			chkbx = document.getElementById('sg_' + snmp_query_id + '_' + frmChk.elements[i].name.substr(prefix.length));
			//chkbx.style.visibility = 'visible';
			chkbx.checked          = false;
		}
	}
...
function SelectAll(prefix, checkbox_state) {
	...
	for (var i = 0; i < frmChk.elements.length; i++) {
		if ((frmChk.elements[i].name.substr(0, prefix.length) == prefix) &&
                    (frmChk.elements[i].style.display != 'none')) {
			frmChk.elements[i].checked = checkbox_state;
		}

		if (prefix == "chk_") {
			lineid = document.getElementById('line'+ frmChk.elements[i].name.substr(4));
			if (lineid) {
				if (frmChk.elements[i].checked) {
